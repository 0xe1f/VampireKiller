...
name: msx-gfx-sheets
description: >-
  Build labelled PNG contact sheets for MSX tiles, hardware sprites, and
  fonts. One tile, 16x16 sprite plane, or glyph per cell, header id,
  in-game palette. gfx/sprites/ is packed 1bpp sprite-asm sheets; gfx/fonts/
  is 1bpp font asms; composites live in gfx/. Use when adding or changing
  gfx/sprites, gfx/tilesets, gfx/fonts, enemy_sheet, sheet_enemy, make gfx,
  gfxdump.py, or sprite/tile/font previews.
---

# MSX gfx contact sheets

Canonical sheets: `gfx/tilesets/tileset_s01.png` and
`gfx/sprites/enemy_sprite_rle.png`. New *catalog* dumps must match those, not
`simon_rle` / `intro_sky`. Implement in `tools/gfxdump.py`; `make gfx`
regenerates. Compressed bytes in `segments/data/*.asm` stay authoritative
(`make verify`). PNG is preview only.
...
Do not put composites or fonts in `gfx/sprites/`. A new packed sprite asm
...
Index 0 stays off (`OFF`), not pal[0].
...
1. Identify the asm (`segments/data/`) and the atom (8×8 tile, 16×16
   tile, 16×16 sprite plane, or 8×8 glyph).
2. Parse that file. Tiles: `parse_asm_tile8` (`; 0xXXXX  … tile …` or
   `… 16x16 …`; skip `rest of` prefixes). Sprite RLE:
   `_parse_asm_sprite_rle` + `rledec.decompress`; skip `*unid*` (not a
   valid stream). Split on 32-byte (or 128-byte) boundaries; do not emit
   a cell for a partial leftover. Fonts: 8-byte glyphs from the asm /
   ROM slice (`dump_credits_font` / `dump_hud_font` / `dump_logo_font`).
...
4. `render_png(..., labels=..., scale=8)` into `SPRITE_DIR` (packed
   sprite asm) or `TILESET_DIR` (4bpp tiles). Fonts: `FONT_DIR`, scale
   12. Packed SAT composites use `render_packed_png` into `GFX`. Hook
   from `main()` / `dump_asm_sprite_rles` / `dump_asm_tilesets` /
   `dump_enemy_frames`.
5. File header comment on the asm: `Preview: gfx/…/<stem>.png`; cell
   header meaning. One line in `docs/game-notes.md` gfx catalogue.
6. `make gfx` and check: every cell labelled, colours match in-game,
   no duplicate header for different art.
...
- **Tiles** — `gfx/tilesets/<stem>.png` ↔ `segments/data/<stem>.asm`
  ...
  keys/portrait). Metatile tables (`mtile_*`) are not this set.
  Trailing `0xFF` bank pads and partial last tiles are leftover, not
  missing cells; do not invent a header for them.
