...
;    tools/workbench/msx/regen-bank.sh 0 0x4000 segments/banks_0123.blocks
;    tools/workbench/msx/regen-bank.sh 1 0x6000 segments/banks_0123.blocks
;    tools/workbench/msx/regen-bank.sh 2 0x8000 segments/banks_0123.blocks
;    tools/workbench/msx/regen-bank.sh 3 0xA000 segments/banks_0123.blocks
...
gm_psg_save_mute:
	...
	call RDPSG
	ld (0e613h),a
; gm_psg_mute (seg0 0x411F): zero PSG volume registers 8-10.
gm_psg_mute:
	ld e,000h               ; volume 0
	ld a,008h               ; ch A
	call WRTPSG
	...
	inc a                   ; ch C
	jp WRTPSG
; gm_psg_restore (seg0 0x4132): restore PSG volumes from E611-E613.
gm_psg_restore:
	ld a,(0e611h)           ; ch A
	ld e,a
	ld a,008h
...
intro_walk_odd:
	...
	call sat_vram_hide
	xor a
	ld (flag_intro_map),a          ; intro map stream off
	call play_hud_reset
	ld hl,0e604h
	ld a,(hl)
	or a
...
state_stage_bridge:            ; 4 (0x422C)
	...
	call vdp_screen_off
	call hud_draw_all
	call play_screen_build
	ld hl,stay_in_play
	ld (hl),001h           ; stay in play (0 = attract/death leave-play)
	call stage_bgm_play
	xor a
	ld (replay_bgm),a          ; clear BGM force-replay (set on death / GM stage jump)
main_state_inc_20:
	ld a,020h
main_state_inc:
	ld (0c004h),a
...
frontend_input:
	...
	ld hl,title_btn_held            ; 0xC401 = title held; 0xC400 = new-press
...
state_pause:                   ; 11 (0x43E1): F1 froze play (0xC40A); wait F1 (0xC00B bit0) to resume
	...
	xor a
	ld (pause_latch),a          ; clear pause latch
...
; in seg2, which is paged at 0x8000): vendor_purchase_tick (0x94C1),
; vendor_offer_dismiss (0x950E), vendor_make_offer (0x938E).
vendor_hold:                   ; (0x4402) C001=2: wait C004, then dismiss
	djnz vendor_begin
	ld hl,0c004h
	dec (hl)               ; hold the offer for 0xC004 frames...
	ret nz
	call vendor_offer_dismiss ; wipe bubble, restore candle outlines
	ld a,main_play
	jp main_state_set
vendor_begin:                  ; (0x4411) C001=0: arm a sale
	xor a
	ld (vendor_hit),a          ; clear the whip-hit flag
...
gm_menu_setup:
	...
	ldir
	jp main_phase_next
; pause_panel_draw (seg0 0x449C): stash 48x16 at (0x68,0x54), frame, "PAUSE".
pause_panel_draw:
	ld hl,06854h            ; playfield dest (X,Y)
	ld bc,03010h            ; 48 x 16
	ld de,0d070h            ; page-1 stash
	ld a,004h               ; HMMM page0->1
	push hl
	push bc
	call vdp_hmmm
	pop bc
...
pause_text:
	...
	vk "PAUSE"
	defb 0ffh
; pause_panel_restore (seg0 0x44BF): HMMM the stashed 48x16 back over the playfield.
pause_panel_restore:
	ld de,06854h            ; playfield dest
	ld bc,03010h            ; 48 x 16
	ld hl,0d070h            ; page-1 stash
	ld a,001h               ; HMMM page1->0
	jp vdp_hmmm
...
reset_run_state:
	ld hl,score           ; HL -> first byte to clear
	...
	ld de,lives           ; ...into 0xC410..0xC412
	ld bc,00003h
	ldir
	ld a,020h
	ld (health),a          ; Simon health = full (0x20); runtime-confirmed HP
	ld a,080h
...
hud_draw_all:
	ld hl,hud_status_str    ; HUD label strings (vk-encoded)
	call hud_string_draw
	call draw_stage_hud     ; STAGE BCD
	call draw_hearts_hud    ; heart count
	call draw_lives_hud     ; PLAYER
	call hud_bars_redraw    ; health / enemy meters
	call hud_panel_frames   ; three boxed outlines
	call hud_weapon_icon    ; current whip/subweapon
	call hud_keys_refresh   ; yellow/lockpick lamp, then white-key lamp
	call hud_bonus_refresh  ; last-collected item
draw_score_hud:                 ; score BCD at (0x38,0) via hud_bcd_draw
	ld hl,score_hi          ; high pair first
	ld de,03800h            ; HUD dest (X,Y)
	ld b,003h               ; 3 BCD pairs
	jr hud_bcd_draw
; --- draw_stage_hud (seg0 0x4542) - draw the HUD STAGE number (0xC411, BCD).
draw_stage_hud:
	ld de,09c00h            ; HUD cell for the stage readout
	ld hl,hud_stage         ; stage/area label (packed BCD)
	ld b,001h               ; 1 BCD byte
	jr hud_bcd_draw
...
hud_panel_frames:
	ld hl,07f0bh            ; (X,Y) weapon panel
	ld de,01212h            ; 18 x 18
	ld c,008h               ; colour 8
	call vdp_box
	ld hl,0930bh            ; key panel
	ld de,02212h            ; 34 x 18
	ld c,00eh               ; colour 14
	call vdp_box
	ld hl,0b70bh            ; bonus panel
...
draw_hearts_hud:
	...
	jr hud_bcd_1byte
; draw_lives_hud (seg0 0x4575): LIVES BCD at (0xE4,0x00).
draw_lives_hud:
...
hud_bars_redraw:
	call health_bar_redraw  ; Simon's bar
enemy_meter_redraw:
	call enemy_meter_frame
	jp draw_enemy_meter     ; fill the enemy/boss meter
; health_bar_redraw (seg0 0x45C0): frame + fill Simon's HEALTH bar.
health_bar_redraw:
	call health_bar_frame
	jp draw_health_bar
; enemy_meter_frame (seg0 0x45C6): clear + border the enemy meter panel (66x6).
enemy_meter_frame:
...
; health_bar_frame (seg0 0x45CF): clear + border the health bar panel (66x6).
health_bar_frame:
...
draw_enemy_meter:
	ld hl,enemy_meter           ; enemy/boss energy (0..0x80)
	ld a,(hl)
	ld hl,03c17h
	ld b,a
	and 003h
...
restore_enemy_meter:
	ld hl,enemy_meter           ; enemy/boss energy
	ld a,(hl)
	add a,b
	cp 081h
...
damage_enemy_1:
	ld b,001h
	jr damage_enemy
; vram_read (seg0 0x4661): VRAM HL -> CPU DE via INIR.  BC = count.  Uses
; BIOS 0x0006 (VDP read port).  vram_otir_count splits into A pages + B rem.
vram_read:
	call vdp_set_read       ; point VDP read at HL
	call vram_otir_count    ; A = pages, B = remainder; HL = CPU dest
	ex af,af'
	ld a,(00006h)           ; VDP read port
	ld c,a                  ; INIR port
	ex af,af'
vram_read_page:                 ; INIR one page; DEC A
	...
	ex de,hl                ; DE = CPU dest end
	ret
; vram_otir_count (seg0 0x4674): after `ex de,hl`, split BC into A = 256-byte
; pages and B = remainder.  If C==0, A is exactly B; else A = B+1 for the partial.
vram_otir_count:
	ex de,hl                ; HL = dest, DE was VRAM
	ld a,c
	or a                    ; remainder 0?
	ld a,b                  ; A = full pages
	ld b,c                  ; B = remainder for OTIR/INIR
	ret z
	inc a                   ; extra pass for the leftover
	ret
...
vram_write_page:                ; OTIR one page; DEC A
	...
	ret
; vram_fill (seg0 0x468F): fill VRAM HL with A; BC = count (B pages, C rem).
vram_fill:
	push de
	push af
	call vdp_set_write
	...
	inc b                   ; extra pass for remainder
...
vdp_set_write:
	...
	ei
	ret
; vdp_set_read (seg0 0x46D5): set VDP VRAM read pointer from HL (R14, no bit6).
vdp_set_read:
	push bc
	ld a,(00007h)           ; VDP control port + 1
	inc a
	ld c,a                  ; 0x99
	ld a,h
	rlca
	...
	out (c),a               ; VRAM pointer lo
	ld a,h
	and 03fh                ; A13-A8, read mode (no bit6)
	out (c),a
	pop bc
	ei
	ret
...
;     Tools: tools/workbench/konami/rledec.py replays this exact grammar to extract graphics.
rle_dec_addr:
...
gfx_script_run:
	...
	dec a
	or a
	jr z,gfx_script_rle_go  ; 0 = RLE
	dec a
	jr z,gfx_script_convert_go ; 1 = sprite convert
	call gfx_script_copy    ; else VRAM blit
	jr gfx_script_run
gfx_script_rle_go:
	call gfx_script_rle
	jr gfx_script_run
gfx_script_convert_go:
	...
	jr gfx_script_run
; gfx_script_rle (seg0 0x4735): cmd 0: src word, VRAM dest word -> rle_dec.
gfx_script_rle:
	ld e,(hl)               ; src lo
	inc hl
	ld d,(hl)               ; src hi
...
gfx_script_convert:
	...
	pop hl
	ret
; gfx_script_copy (seg0 0x4772): cmd 2+: 6-byte VRAM blit (vram_write).
gfx_script_copy:
	ld e,(hl)               ; src lo
	inc hl
	ld d,(hl)               ; src hi
	...
	inc hl                  ; skip dest hi
	ret
; gfx_1bpp_expand (seg0 0x4786): expand A planes from 0xE800 -> 0xEC10.
gfx_1bpp_expand:
	ld hl,0e800h            ; 1bpp source
	ld de,0ec10h            ; 4bpp dest
	ld c,a                  ; plane count
...
gfx_1bpp_plane_lo:              ; second plane after DE wrap
	...
	ret
; gfx_1bpp_row (seg0 0x47A4): 16 source bytes at HL -> 16 dest bytes at DE.
; Each dest byte is `1 << (7 - index of the highest set bit)` of the source
; (a 1bpp "which ink" encode used by the sprite-convert path).
gfx_1bpp_row:
	ld b,010h               ; 16 bytes
...
gfx_1bpp_shift:                 ; rotate until MSB set
	...
	djnz gfx_1bpp_byte
	ret
; video_clear_page (seg0 0x47B8): hide SAT, HMMV 256x256 colour 0 from (0,0)
; page 0 (BC=0 means NX/NY high-bit 256), then screen on.
video_clear_page:
	call sat_vram_hide
	ld bc,00000h            ; 256 x 256
	jr video_fill_go
...
; vdp_screen_on (seg0 0x47CE): set RG1 bit6 (display enable), then vdp_sprites_on.
vdp_screen_on:
...
; vdp_fill_origin (seg0 0x47E8): HMMV from (0,0) page 0 size BC, then R23=0.
vdp_fill_origin:
...
vdp_line_v_save:
	...
	pop hl
	ret
; vdp_line_h_save (seg0 0x4907): vdp_line_h, preserving HL/DE/BC.
vdp_line_h_save:
	push hl
	push de
	push bc
...
vdp_hmmc:
	...
	ld a,0ach               ; R17 = 44 (CLR), auto-inc OFF
	out (c),a               ; R17 = 44 (CLR), auto-increment OFF
	ld a,091h               ; commit R17
	out (c),a
	inc c
	inc c                   ; 0x9B again
...
hud_glyph_run:
	...
	djnz hud_glyph_run
	ret
; tile_blit (seg0 0x4B12): HMMM 8x8 tile A from the page-1 atlas to DE page 0.
tile_blit:
	push bc
	push hl
	push de
	call tile_atlas_pos     ; HL = atlas (SX,SY) for tile A
	ld bc,00808h            ; 8x8
	ld a,001h               ; page1 -> page0
	call vdp_hmmm
	pop de
	pop hl
	pop bc
...
play_input:
	ld a,(0c002h)
	and 040h                ; bit6 = live input
	jp z,attract_input      ; attract: scripted buttons
	call read_fkeys
	ld hl,0c00ch            ; F-key held
	call input_edge
	call read_buttons
input_edge_play:
	ld hl,btn_held           ; play: held at C007, rising edge at C006
; input_edge (seg0 0x4BBB): A = this frame's buttons, HL -> held byte.
; Stores A at (HL) and the rising edge (new presses) at (HL-1).
input_edge:
	ld c,(hl)               ; previous held
...
read_buttons:
	ld e,08fh               ; PSG R15 = 0x8F (joystick port A)
	ld a,00fh
	call WRTPSG
	ld a,00eh               ; read R14
	di
	call RDPSG              ; joystick bits (active low)
	ei
	cpl
	and 03fh                ; stick + 2 triggers
	push af
	ld a,008h               ; keyboard row 8
	call SNSMAT
	cpl
	ld e,a
	and 020h                ; SPACE
	...
	cpl
	rrca
	rrca                    ; shift arrows toward stick bits
	ld b,a
	and 004h                ; LEFT
	or e
	ld c,a
	...
	rrca
	ld b,a
	and 018h                ; UP / DOWN folded
	or c
	ld c,a
	ld a,b
	rrca
	and 003h                ; RIGHT leftover
	or c
	pop bc                  ; joystick
	or b                    ; OR keyboard
	ret
...
read_fkeys:
	...
	and 007h
	ret
; --- hud_status_str (0x4C07): HUD / status-bar text.  vk "TEXT" is ASCII-0x10
...
title_prompt_str:              ; (0x4D0F) "KONAMI 1986" / "PUSH SPACE KEY"
	...
	vk "PUSH SPACE KEY"
	defb 0ffh
play_start_str:                ; (0x4D30) "PLAY START"
	defb 048h, 0a0h, 000h, 000h
	vk "PLAY START"
	defb 000h, 000h, 0ffh
game_over_str:                 ; (0x4D41) "GAME  OVER"
	defb 058h, 058h
	vk "GAME  OVER"
	defb 0ffh
...
title_build:
	...
	ld b,003h               ; 3 fade steps
	ld de,06606h            ; rb,g start
...
title_delay:                    ; busy-wait between palette steps
	...
	ld hl,00010h            ; dest (0,0x10)
	ld bc,00068h            ; 256 x 104
	ld a,0ffh               ; fill
	ld d,000h               ; page 0
	call vdp_hmmv
	ld hl,00516h
...
vram_clear_page_go:
	...
	xor a
	jp vdp_hmmv
; attract_run_init (seg0 0x4DEE): wipe play RAM from C420, seed stage 1, HUD.
attract_run_init:
	ld hl,simon_action      ; start of Simon/play block
	ld de,simon_jump_dir    ; ldir dest = C421
	ld bc,01bdfh            ; through 0xDFFF
	xor a
	ld (hl),a               ; seed zero
	ldir
	ld (btn_held),a
	...
	ld (hub),a
	inc a
	ld (stay_in_play),a     ; stay in play
	ld (hud_stage),a        ; STAGE = 1
	ld (stage),a
	ld (0cf3ah),a           ; attract input timer
	ld a,020h
	ld (health),a           ; Simon health = full (0x20)
	ld a,080h
	ld (enemy_meter),a      ; enemy/boss energy meter = full (0x80)
	...
	call play_screen_build
	jp hud_draw_all
; attract_tick (seg0 0x4E27): play_tick; on exit_dir, follow CONN and redraw.
attract_tick:
	call play_tick
	ld a,(exit_dir)
	and a
	ret z                   ; still in the room
	call conn_lookup_paged
...
intro_actors_frame:
	...
	call c800_sat_build
	call c800_sat_emit
	jp pattern_shadow_blit  ; blit pattern shadow to VRAM
...
intro_sky_init:
	ld (ix+006h),001h
	ld (ix+00bh),pose_intro_sky      ; pose_intro_sky
	ld (ix+00eh),a
	ret
intro_sky_go:
	xor a
...
intro_sky_ab_vel:               ; Y/X vel after type-bit0 flip
	...
	call actor_set_xvel
	ld (ix+006h),001h
	ld (ix+00bh),pose_intro_sky_a      ; pose_intro_sky_a
	xor a
	ld (ix+010h),a
	ld (ix+00eh),a
	ret
intro_sky_ab_go:
	...
	bit 0,(ix+011h)
	ret z
	ld (ix+00bh),pose_intro_sky_b      ; pose_intro_sky_b
	ret
...
intro_spawn_simon:
	...
	jp spawn_actor
intro_simon_init:
	ld (ix+00bh),pose_intro_simon_1      ; pose_intro_simon_1
	xor a
	ld (ix+001h),a
	ld (ix+013h),a
	ld (ix+014h),a
...
intro_simon_idle:              ; (0x4F5E) X < 0x80: stop, pose 0x97
	ld (ix+006h),000h
	ld (ix+00bh),pose_intro_simon_0      ; pose_intro_simon_0
	ret
...
playfield_draw:
	ld hl,0d140h            ; nametable row 2 (skip 2 HUD rows)
	ld de,00020h            ; dest (X=0, Y=0x20)
	ld b,016h               ; 22 tile rows
...
playfield_tile:                 ; one of 32 tiles in the row
	...
	inc hl
	ld a,d
	add a,008h              ; next cell X
	ld d,a
	djnz playfield_tile
	pop bc
	ld a,e
	add a,008h              ; next row Y
	ld e,a
	djnz playfield_row
	ret
...
page_play_banks:
	...
	ei
	ret
; --- page_sound_banks - PSG driver window: seg 14 @ 0x8000, seg 15 @ 0xA000.
;     Shadows at 0xF0F2/F0F3.  play_sound does the same pair inline.
page_sound_banks:
	di
	ld hl,0f0f2h            ; shadows for pages 2a/2b
	ld a,00eh               ; bank 14 -> 0x8000
	ld (08000h),a
	ld (hl),a               ; shadow 0xF0F2
	inc l                   ; 0xF0F3
	inc a                   ; bank 15
...
page_title_banks:
	...
	ei
	ret
; --- page_tileset_late - stages 13+: overlay seg 7 @ 0x8000, seg 8 @ 0xA000
;     on the tileset window (page_tileset_banks already put 4/5/6; this
;     replaces 5/6).  Shadows at 0xF0F2/F0F3.
page_tileset_late:
	di
	ld hl,0f0f2h            ; shadows for pages 2a/2b
	ld a,007h               ; bank 7 -> 0x8000
	ld (08000h),a
	ld (hl),a               ; shadow 0xF0F2
	inc l                   ; 0xF0F3
	inc a                   ; bank 8
...
vdoor_sat_y:                    ; SAT Y = door Y, +0x10 if B<5
	...
	ld hl,0d430h
	ld c,002h
; sat_color_fill (seg0 0x5642): write C to 16 bytes at HL and at HL+0x40
; (sprite colour table + the CC-linked pair).  Falls in from the last
; vdoor SAT colour (D430, colour 2).
sat_color_fill:
	ld d,h
	ld e,l
	ld a,040h               ; CC pair is +0x40
	call ADD_DE_A
	ld b,010h               ; 16 lines
...
load_simon_sprites:
	...
	ld a,(simon_legs)       ; legs frame index
	...
	ld a,(simon_torso)      ; torso/whip frame index
	add a,a                 ; *2
	...
	call rle_dec
	jp page_play_banks      ; restore default game banks
; stage_palette_load (seg0 0x5714) - HUD-fixed colours, then stage_palette_ptr[stage].
;  Room entry overlays the per-room table via room_gfx_load.
stage_palette_load:
	call palette_hud_load
	call page_title_banks
...
tileset_ptr:
	...
	defw 09640h
	defw 0a4c0h            ; 18 (seg8)
; load_shared_sprites (seg0 0x576F) - fireball/flame (gfx_rle_a185 @ VRAM 0xFF00,
;  SAT pat 0xF0) and gfx_rle_a147 @ 0xF9C0.  Called from play_hud_reset.
load_shared_sprites:
	call page_title_banks
...
dracula_body_unpack:
	ld b,020h               ; 32 rows
	ld de,0e800h            ; unpack dest
body_row:
	push bc
	ld a,(hl)               ; N = leading zeros
	and a
	jr z,body_copy
	ld b,a
	xor a
body_zeros:
	...
	djnz body_zeros
body_copy:
	ld a,00ch               ; 12-byte payload row
	sub (hl)
	inc hl                  ; skip the count
	ld c,a                  ; remaining payload
	ld b,000h
	ldir
	xor a
	ld b,004h               ; 4 trailing zeros
body_pad:
	ld (de),a
	inc de
	djnz body_pad
...
dracula_body_hflip:
	ld hl,0e800h            ; left edge of the 32x32
	ld de,0e80fh            ; right edge of row 0
	ld c,020h               ; 32 rows
body_hflip_row:                 ; one of 32 rows nibble-swap
	ld b,008h               ; 8 byte-pairs
	call nibble_hflip
	ld a,008h               ; next row left
	call ADD_HL_A
	ld a,018h               ; next row right (16+8)
	call ADD_DE_A
	dec c
	jr nz,body_hflip_row
	ret
; nibble_hflip (seg0 0x5873): 4bpp horizontal flip of B bytes.  Swap the
; nibbles of (HL) with (DE), then HL++, DE--.  Dracula body/portrait mirror.
nibble_hflip:
	ex af,af'
	ld a,(hl)               ; left byte
	ex af,af'
	...
	rrca
	rrca
	rrca                    ; swap its nibbles
	ld (hl),a               ; -> left
	ex af,af'
	rrca
	rrca
	rrca
	rrca                    ; swap the saved left
	ld (de),a               ; -> right
	inc hl
	dec de                  ; walk inward
	djnz nibble_hflip
...
dracula_portrait_hflip:
	...
	ld b,002h
	ld de,08048h
; dracula_hflip_run (seg0 0x58F1): B tiles: hflip-blit from HL to atlas DE,
; then atlas_advance_x4.  Falls in from the last portrait-hflip call.
dracula_hflip_run:
	push bc
	push de
	push hl
	...
	pop hl
	ld de,00020h            ; next 8x8 source (32 bytes)
	add hl,de
	pop de
...
dracula_portrait_vflip:
	...
	ld b,002h
	ld de,08068h
; dracula_vflip_run (seg0 0x594F): B tiles: vflip-blit from HL to atlas DE,
; then atlas_advance_x4 on both dest and source.  Falls in from the last call.
dracula_vflip_run:
	push bc
	push de
	push hl
	...
	ret
; atlas_advance_x4 (seg0 0x5962): SCREEN 5 tile-atlas dest DE: X += 4 bytes
; (8 px).  Wrap at the 0x80-byte scanline and drop 8 px (D += 4, 4-aligned).
atlas_advance_x4:
	ld a,e
	add a,004h              ; +8 pixels
	and 07fh                ; stay in the 128-byte line
	ld e,a
	ret nz                  ; still on this row
	ld a,d
	add a,004h              ; next 8 px of atlas Y
	and 0fch                ; 4-aligned
	ld d,a
	ret
tile8_vflip_blit:
...
conn_lookup_paged:             ; 0x5A35
	...
	call conn_lookup        ; this room's CONN word
...
; scenery_item_xy (seg0 0x5B9D): HL -> byte after the packed pos.  Back up,
; scenery_pos_xy -> DE=(X,Y), then B = next byte & 0x1F (item / display id).
scenery_item_xy:
	dec l                   ; back to pos byte
	push hl
	push bc
	call scenery_pos_xy     ; C=Y, B=X; HL++
	ld d,b                  ; D = X
	ld e,c                  ; E = Y
	pop bc
	ld a,(hl)               ; type / item byte
	and 01fh                ; bits4-0 = id
	ld b,a
	pop hl
	ret
...
scenery_pos_xy:
	ld a,(hl)               ; packed pos
	ld b,a
	and 0f0h                ; high nibble = Y
	ld c,a                  ; C = Y pixels
	ld a,b
	and 00fh                ; low nibble = X cell
	add a,a
	add a,a
	add a,a
	add a,a                 ; *16 = X pixels
...
play_tick:
	...
	call minimap_driver
	ld a,(0cf38h)           ; F2 map open?
	and a
	ret nz                  ; freeze play while the map is up
play_tick_live:
	ld a,(0c002h)           ; frame counter
play_tick_c002:
	and 040h
	jr z,play_tick_frame
	ld a,(0c00bh)           ; F-key rising edge
	rra                     ; F1?
...
gm_sig_cmp:
	...
	ld hl,07ffah            ; last 6 bytes of a 16K page
	ld b,006h
gm_sig_loop:
	push bc
gm_sig_rdslt:
	...
	pop bc
	ex de,hl
	cp (hl)                 ; vs expected byte
	ex de,hl
	jr nz,gm_sig_miss
	inc hl                  ; next ROM addr
	inc de                  ; next sig byte
	djnz gm_sig_loop
	scf                     ; match
	ret
gm_sig_miss:
...
gm_box_clear:                  ; HL = (x,y), BC = (w,h): HMMV to 0, keep args
	...
	call vdp_hmmv
	pop hl                  ; restore dest
	pop de                  ; restore size as DE
	ret
; panel_frame (0x5D15): clear then outline the GM box at HL size BC, colour 14.
panel_frame:
	call gm_box_clear
	ld c,00eh               ; colour 14
	jp vdp_box
...
gm_confirm_key:
	...
	call SNSMAT
	cpl
	and 080h                ; bit 7 = RETURN
	ld hl,0e60ah            ; previous sample
	ld c,(hl)
	ld (hl),a
	xor c
	and (hl)                ; press edge
...
gm_apply_bcd:
	...
	call ADD_HL_A
	ld a,(hl)
	ld (hub),a          ; hub index for the new stage
	xor a
	ld (room),a          ; room = 0
	inc a
...
room_spawner:
	ld a,(rosary_timer)    ; rosary / pickup no-spawn
	and a
...
; spawn_actor_ab (seg0 0x5F26): same as spawn_actor but keep A -> CFFA and
; B -> CFFB (drop gate / extra).  Scan 7 C800 slots, claim SAT, init, DISPATCH.
spawn_actor_ab:
	ld (0cffah),a           ; extra A
	ld a,b                  ; extra B
	ld (0cffbh),a           ; stash
...
;  Regen: tools/workbench/msx/regen-bank.sh 1 0x6000 segments/banks_0123.blocks
; ===========================================================================
...
object_list_lookup:
	ld a,(hub)          ; A = current cell/level index
...
object_list_spawn:
	ld a,(hub)          ; A = level index
	ld c,a
	add a,a
	add a,c                ; C = index*3 (stride of the row block)
	ld c,a
	ld a,(stage)          ; A = current row/scroll
	dec a
	...
	ld a,(room)          ; A = column within the row
	add a,a
	add a,a
	add a,a
...
konami_logo_step:
	ld hl,simon_action            ; HL -> 0xC420 frame divider
...
;  attract/game-start call this before play_screen_build.
play_hud_reset:
	ld a,020h               ; full HP
	ld (health),a           ; Simon health = full (0x20)
; play_hud_meter (seg1 0x62DC): play_hud_reset without rewriting health.
play_hud_meter:
	ld a,080h
	ld (enemy_meter),a      ; enemy/boss energy meter = full (0x80)
	call inv_reset_life     ; C700 block / lives lamps
	call hud_cache_load     ; page-1 HUD tiles
	call load_shared_sprites
play_hud_font:
	jp hud_font_load

; --- play_screen_build (seg1 0x62ED) - build a gameplay cell -----------------
;  Called from state_stage_bridge and game-start.  Clears per-screen state,
;  paints tiles, sets the cell event, unpacks scenery, loads the object list
;  and spawns its actors.  Attract room-changes jump in at play_screen_redraw
;  (skip tileset / Simon spawn / scenery).
play_screen_build:
	call load_stage_tileset
	call stage_palette_load
...
; play_screen_redraw (seg1 0x62FC): sprites_hide onward (room change / attract).
play_screen_redraw:
	...
	cp evt_dracula
	call z,dracula_face_rest       ; event 6 -> closed mouth + eyes
	call vdp_screen_on
	call spawn_rate_reset
	call object_list_load   ; packed object list into 0xDB00/DC00/DD00
...
;  up the current map cell in cell_event_tbl[stage]: the byte's high nibble is
;  the room it applies to and the low nibble is the event code.  If the high
;  nibble matches the current room (0xD001) the event code is stored in 0xCE00;
;  code 6 additionally kicks off its handler (seg0 dracula_portrait_load 0x5887,
;  plus 0x59F3 / tail-call dracula_body_load 0x57E6).
...
; cell_event_tbl (seg1 0x6376) - one byte per stage 0..18.
;  High nibble = room, low nibble = CE00 event (1=bat .. 6=Dracula).  0xFF = none.
cell_event_tbl:
...
intro_scene_build:
	call video_reset
	ld a,080h
	ld (simon_y),a          ; Simon Y = 0x80
	ld (simon_x),a          ; Simon X = 0x80
...
intro_spawn_rest:               ; sky_a/b + Simon, then draw the room
	...
	call playfield_draw
	jp vdp_screen_on
; --- simon_spawn_pos - set Simon's spawn position + facing from the per-stage table
;  Looks up simon_spawn_tbl[stage] (2 bytes): byte0 with bit0 masked off -> Simon Y
;  (0xC425) and bit0 -> facing flag 0xC42C (0=right, 1=left); byte1 -> Simon X
;  (0xC427).
simon_spawn_pos:
	ld a,(stage)          ; A = current stage
	ld hl,simon_spawn_tbl
	add a,a                ; *2 (2 bytes per stage)
	call ADD_HL_A          ; HL -> simon_spawn_tbl[stage]
	ld a,(hl)
	inc hl
	ld c,a
	and 0feh               ; drop bit0 ...
	ld (simon_y),a          ; ... -> Simon Y (0xC425)
	ld a,(hl)
...
simon_spawn_tbl:
	defb 0b0h,010h         ; 0  Y=B0 right X=10
	defb 0c0h,010h         ; 1
	defb 060h,010h         ; 2
	defb 060h,010h         ; 3
	defb 0c0h,010h         ; 4
	defb 061h,0e8h         ; 5  Y=60 left  X=E8
	defb 081h,0e8h         ; 6
	defb 0b1h,0e8h         ; 7
	defb 090h,010h         ; 8
	defb 070h,010h         ; 9
...
shot_sat_emit:
	...
	ld b,008h
	jr sat_emit_loop
; c800_sat_emit (seg1 0x64F3): 7 x C800 SAT blocks -> 0xD638 hardware shadow.
c800_sat_emit:
...
;  at 0xD638 + id*4 and fills 16 SAT-colour bytes at 0xD4E0 + id*16
;  (computed as (0x6A70 + id*8)*2 — 0x6A70 is not a table).
actor_sat_emit:
...
actor_sat_cell:
	...
	add a,a
	add a,a                ; id*4
	ld de,0d638h           ; SAT attr base
	add a,e
	ld e,a                 ; DE -> this sprite's SAT record
	ld c,0ffh
	ldi                    ; Y
	ldi                    ; X
	ldi                    ; pattern
...
dracula_quiet_tick:             ; store CE02; wait slot0 empty
	...
	jp event_ce01_next
event_dracula_theme:
	ld a,bgm_boss_dracula_portrait
...
credits_keyframe:
	...
	call hud_string_glyphs ; blit the line, +8 X per glyph
	ld hl,0ce31h
...
dracula_ce35_opening:
	...
	ld (0ce37h),a
	ret
; dracula_face_close (seg1 0x68AF): CE37 timer.  At 0x10 blit the half-open
; composite; 0x10 frames later (CE37 wraps through sub 0x10) close the mouth.
dracula_face_close:
	ld hl,0ce37h            ; face timer
	inc (hl)
	cp 010h
	...
	ld (0ce36h),a           ; mouth state 0 = closed
...
dracula_face_close_half:
	...
	ld (hl),001h            ; half
	cp 002h
	ret nz                  ; only from fully open
	jr dracula_blit_mouth_half
; dracula_face_open (seg1 0x68CB): CE37 timer.  At 0x10 half-open, at 0x20 full open.
dracula_face_open:
	ld hl,0ce37h
	inc (hl)
	ld a,(hl)
...
dracula_bat_init:
	...
	ld (ix+00bh),pose_dracula_robe_0      ; pose_dracula_robe_0
	ld de,00300h
	call actor_set_yvel
	ld hl,0cf32h
	inc (hl)
	ld a,(hl)
	and 007h
	ld de,dracula_bat_xvel
	call lookup_word_tbl
	ld a,(simon_x)          ; Simon X
	sub (ix+005h)
	call c,neg_de
	jp actor_set_xvel
dracula_bat_xvel:              ; (0x6A93) 8 home-X speeds; index CF32&7
...
grounded_door:
	...
	ret nz
	ld de,00000h
	ld (simon_legs),de         ; walk anim frames (legs, torso)
...
platform_carry_simon:
	dec a                   ; slot id 1/2 -> 0/1
	ld a,007h               ; slot 2 offset
...
; simon_walk_left (seg1 0x6C36): face left, refuse X<0x10 unless left-exit
; is permitted, then probe -X.  Boots (bonus_flags bit3) use -0x280 else -0x200.
simon_walk_left:
	ld a,001h
	ld (simon_facing),a     ; facing = left
	ld a,(permit_left)      ; left exit permit
	inc a                   ; 0xFF blocked -> 0
	jr nz,walk_left_ok      ; permitted (or not 0xFF)
	ld a,(simon_x)
	cp 010h
	ret c                   ; at the wall, no exit
walk_left_ok:                   ; permit or X>=0x10: probe -X
	...
	ld bc,0fe00h            ; -0x200
	jr z,walk_left_vel
	ld bc,0fd80h            ; boots -0x280
walk_left_vel:                  ; BC = -0x200 or boots -0x280
	jr walk_apply
; simon_walk_right (seg1 0x6C5A): face right, refuse X>=0xF0 unless right-exit
; is permitted.  Boots +0x280 else +0x200.  Shared walk_apply: skip if on the
; door or door_state is 1..2, skip walk anim while hurt, then add BC to X16.
simon_walk_right:
	xor a
...
walk_right_ok:                  ; permit or X<0xF0: probe +X
	...
	ld bc,00200h            ; +0x200
	jr z,walk_apply
	ld c,080h               ; boots +0x280 (B already 2)
walk_apply:                     ; shared: door overlap then add X
	...
	ld a,(door_state)
	dec a
	dec a                   ; 1 or 2 -> 0xFF or 0
	cp 002h
	ret c                   ; door_state 2 or 3: don't walk through
	ld a,(simon_action)
	cp act_hurt
	jr z,simon_add_x        ; knockback: skip the walk anim
	dec a
...
simon_land:
	...
	ld (simon_arc),a
	ret
; simon_jump_y_step (seg1 0x6D54): HL -> simon_arc.  Y += table[min(arc,D)-1]
; at BC.  D is the last table index (0x15 on jump, smaller on knockback).
simon_jump_y_step:
	inc (hl)                ; arc++
	ld a,(hl)
	cp d
...
simon_hurt_init:                ; (0x6F9A) sfx, i-frames; stairs if stair_step, else knock
	...
	ld a,(health)
	and a
	jr z,simon_hurt_knock  ; HP 0 -> knockdown (death)
	ld a,act_stairs
	ld (simon_action),a
	ld a,002h
	ld (simon_torso),a
	jp simon_mirror_frames
simon_hurt_knock:               ; (0x6FC3) apply hurt_facing; step 1 (left) or 2
	ld a,(hurt_facing)
	ld (simon_facing),a
	inc a                  ; facing 0 -> step 1 left; 1 -> step 2 right
	...
	ld (simon_legs),de
	jp simon_mirror_frames
simon_hurt_left:                ; (0x6FDB) step 1: knock left
	call simon_walk_left
simon_hurt_arc:                 ; (0x6FDE) shared knock arc; land -> step 3
	ld bc,arc_dy_tbl
	ld d,015h
	...
	ret c
	call simon_floor_test
	jr c,simon_hurt_land
	ld a,(simon_on_plat)
	and a
	ret z
simon_hurt_land:                ; (0x6FF9)
	ld a,(simon_y)
	and 0f8h               ; snap Y to 8px
	ld (simon_y),a
	ld a,003h
	ld (simon_hurt_step),a ; slide
	ld a,(health)
	and a
...
hurt_knock_store:               ; C42A = 4 alive / 0x10 dead
	...
	ld (simon_legs),de
	ret
simon_hurt_right:               ; (0x701E) step 2: knock right
	call simon_walk_right
	jp simon_hurt_arc
simon_hurt_slide:               ; (0x7024) step 3: countdown C42A, then clear hurt
	ld a,(simon_on_plat)
	and a
	call nz,platform_carry_simon
	ld hl,simon_knockback
	...
	ld a,(door_state)
	cp 002h                ; idle (tbl 2)
	jr z,simon_hurt_recover
	cp 003h                ; open walk
	jr z,simon_hurt_recover
	cp 005h                ; vertical open walk
	jr z,simon_hurt_recover
	ld a,(simon_facing)
	xor 001h               ; turn around after knock
	ld (simon_facing),a
simon_hurt_recover:             ; (0x705A) if HP, done; else dying
	ld a,(health)
	and a
	ret nz
	ld hl,simon_x
	ld a,(hl)
	cp 010h
	jr nc,simon_dying_enter
	add a,008h             ; nudge off the left edge
	ld (hl),a
simon_dying_enter:              ; (0x706A) act_dying; jump_dir reused as phase 0
	xor a
	ld (simon_invuln),a
	ld (simon_jump_dir),a  ; dying phase 0
	inc a
	ld (simon_arc),a       ; 1 frame, then death BGM
	ld a,act_dying
	ld (simon_action),a
	ld bc,00509h           ; death pose (legs/torso)
	ld (simon_legs),bc
	jp simon_mirror_frames
...
simon_dying:                    ; 6 (0x709A) jump_dir = dying phase (not jump)
	...
	ld hl,simon_arc
	and a
	jr nz,simon_dying_hold ; phase 1: hold death BGM
	dec (hl)
	ret nz
	ld (hl),05ah           ; 90 frames of death BGM
	ld a,001h
	ld (simon_jump_dir),a  ; phase 1
	ld a,sound_stop
	call play_sound
	ld a,bgm_simon_death
	jp play_sound
simon_dying_hold:               ; (0x70B6) then clamp hearts to 5
	ld hl,simon_arc
	dec (hl)
	ret nz
	ld de,hearts
	ld a,(de)
	cp 005h
	jr c,simon_dying_leave
	ld a,005h              ; death penalty: hearts max 5
	ld (de),a
simon_dying_leave:              ; (0x70C6) wipe timers; stay_in_play=0 -> death_respawn
	xor a
	ld (hl),a
	ld (simon_jump_dir),a
	ld (simon_whip),a
	...
	ld (rosary_timer),a
	ld (0d010h),a          ; hourglass / whip freeze
	ld (room),a
	ld (stay_in_play),a    ; leave play (death)
...
whip_begin:                     ; (0x713A) whip phase 1
	ld (hl),001h
	ret
simon_try_air_item:            ; (0x713D) jump + C701: holy water then hourglass
	ld a,(simon_action)
	dec a
...
; simon_try_air_item while jumping (C420==1) and SPACE is not a new-press.  Needs a DOWN
; new-press (C006 bit1), 5 hearts, and hourglass_timer==0.  Arms hourglass_timer
; (0x5A ~1.5s, or 0x96 ~2.5s with bonus_flags bit2) and D010 bit0, which skips enemy AI/movement.
hourglass_use:
	...
	ld a,(bonus_flags)
	and 004h               ; bit2 = tipped hourglass
	ld a,096h              ; 150 frames ~2.5s
	jr nz,hourglass_arm
	ld a,05ah              ; 90 frames ~1.5s
hourglass_arm:                  ; (0x718E) store timer, freeze actors, BGM 0xFB
	ld (hourglass_timer),a
	ld hl,0d010h
	set 0,(hl)
	ld a,sound_fb
...
; Knife (swing_weapon==knife) may occupy both slots; axe/cross take one.
projectile_alloc:
	ld a,(swing_weapon)
	cp equip_knife
	ld b,002h              ; knife: try both slots
	jr z,projectile_alloc_go
...
torso_store:                    ; leather 6 / chain 9 then mirror
	...
	call play_sound
	jr whip_phase_next
whip_thrown_pose:               ; (0x7231) knife/axe/cross torso 0x0C
...
whip_phase_next:                ; (0x7242)
	...
	inc (hl)
	ret
whip_phase_2:                   ; (0x7247) inc torso; thrown: reload SAT
	ld hl,simon_torso
	inc (hl)
	call whip_phase_next
	ld a,(swing_weapon)
	cp equip_knife
	ret c
	jp load_weapon_sprites
whip_phase_3:                   ; (0x7257) melee: rearm, torso++, timer=4; thrown: phase 2
	ld a,(swing_weapon)
	cp equip_knife
	jr nc,whip_phase_2
	call actors_rearm_hittable
	...
	ld a,004h
	jr whip_timer_set
whip_phase_4:                   ; (0x7269) thrown: projectile_arm; melee: wait timer
	ld a,(swing_weapon)
	cp equip_knife
...
simon_attack_end:
	...
	cp act_stairs
	ld a,000h
	jr nz,whip_idle_torso
	ld a,002h              ; on stairs: keep climb torso
whip_idle_torso:                ; (0x7287)
	ld (simon_torso),a
	jp simon_mirror_frames
projectile_arm:                 ; (0x728D) phase 4: copy swing_weapon into waiting slot
	call actors_rearm_hittable
...
projectile_clear:
	push ix
	pop hl
; projectile_clear_hl (seg1 0x74F3): free the 16-byte C450/C460 knife/axe/cross
; slot at HL, hide its SAT pair (Y=0xE0) and the slot+4 hide-Y.
projectile_clear_hl:
	push hl
	ld (hl),000h            ; type = free
	ld d,h
	ld e,l
	inc de                  ; rest of the 16-byte slot
	ld bc,0000fh
	ldir                    ; zero +1..+15
	pop hl
	ld a,l
...
proj_hide_pat:                  ; slot+4 = hide Y
	...
	ld (hl),a               ; +4 hide-Y
	ret
; whip_slots_clear (seg1 0x751A): zero C450 and C460 (32 bytes) and hide both
; SAT pairs plus the slot+4 hide-Y (0xE0).
whip_slots_clear:
	ld hl,0c450h
	ld (hl),000h            ; C450 type = free
	ld d,h
	ld e,l
	inc de
	ld bc,0001fh            ; rest of C450+C460
	ldir
	ld a,0e0h               ; hide Y
	ld (0c454h),a           ; C450 +4
	ld (0c464h),a           ; C460 +4
	...
	ld (0d624h),a
	ret
projectile_sat:                ; (0x753C) C450/C460 -> SAT D618/D620 + colour
	ld a,(ix+000h)         ; slot live?
	and a
	ret z
	push ix
	pop hl                 ; HL = slot (C450 L=0x50 / C460 L=0x60)
	ld a,l
	cp 050h                ; C450 vs C460
...
proj_sat_dest:                  ; write Y/X/pat pair at HL
	...
	ld (hl),a
	inc hl
	ld a,(ix+006h)         ; pattern
	ld (hl),a
	inc hl
	inc hl                 ; skip SAT colour byte (CC lives in D4xx)
	ld a,(ix+004h)
	sub 010h               ; second cell: same Y
	ld (hl),a
	inc hl
	ld a,(ix+005h)
	sub 008h               ; same X
	...
	inc hl
	ld a,(ix+006h)
	add a,004h             ; second cell pattern = first + 4
	ld (hl),a
	push ix
	pop hl
	ld a,l
...
proj_sat_col:                   ; colour run at D460 or D480
	...
	ld a,00fh              ; type 4: colours 0x0F then 0x0E (fall into sat_fill)
	ld b,010h
	call sat_fill
	ld a,00eh
	ld b,010h
; sat_fill (seg1 0x7597): fill B bytes at HL with A (one SAT colour run).
sat_fill:
	ld (hl),a               ; colour
	inc hl
	djnz sat_fill
	ret
projectile_sat_ink:            ; (0x759C) knife/axe/holy: 0x02/0x4C; flame = 8
	ld a,002h              ; first 16 colour bytes
	ld b,010h
	call sat_fill
	ld a,04ch              ; second 16: CC bit (0x40) + colour 0x0C
	ld b,010h
	call sat_fill
	ld a,(ix+001h)
	cp 005h                ; holy water?
	...
	ld a,(ix+000h)
	cp 003h
	ret nz                 ; type 5 state 3 = floor flame: colour 8
...
;   sapphire_ring / simon_invuln - per-frame timers (i-frames also portal wind-up).
timers_tick:
	ld hl,rosary_timer     ; rosary / weapon-pickup no-spawn
	call dec_nonzero
	ld hl,sapphire_ring
	call dec_nonzero
	ld hl,simon_invuln     ; i-frames / portal wind-up (falls into dec_nonzero)
; dec_nonzero (seg1 0x75D6): decrement (HL) unless already 0.
dec_nonzero:
	ld a,(hl)
	and a
	ret z                   ; already 0
...
gem_timer_tick:                ; C43A invis; sfx 0x17 (gem warn) at 16 frames left
	...
	ld a,sfx_gem_warn
	jp play_sound
hourglass_timer_tick:          ; hourglass_timer; on 0: res D010.0, restore BGM (0xFC)
	ld hl,hourglass_timer
	ld a,(hl)
	and a
	ret z
	dec (hl)
...
; a stage boundary at door_open_exit/set_stage_boundary.  0xC420 = Simon's action state
...
room_edge_down:                        ; past the bottom edge
	...
	ld (hl),a
	ret
pit_water_sfx:                 ; (0x76EA) stages 2/10 bottomless pit: splash
	ld a,sfx_water_in
	jp play_sound
...
; layer (door_open_exit): blocked permit -> set_stage_boundary / advance_stage; valid
...
door_open_exit:                ; (seg1 0x77D8)
	ld hl,permit_left           ; hl -> left exit permit (0xC41E)
...
set_stage_boundary:
	ld a,001h
	ld (door_exit),a          ; stage-boundary flag -> advance_stage
door_idle:
	ret
...
stage_bgm_tbl:
	defb bgm_s00_03,bgm_s00_03,bgm_s00_03,bgm_s00_03  ; stages 0-3
	defb bgm_s04_06_11_12,bgm_s04_06_11_12,bgm_s04_06_11_12        ; 4-6
	defb bgm_s07_09,bgm_s07_09,bgm_s07_09        ; 7-9
	defb bgm_s10_18                  ; 10
	defb bgm_s04_06_11_12,bgm_s04_06_11_12              ; 11-12
	defb bgm_s13_15,bgm_s13_15,bgm_s13_15        ; 13-15
	defb bgm_s16_17,bgm_s16_17              ; 16-17
	defb bgm_s10_18                  ; 18
...
title_set_color2:
	ld a,002h               ; palette index 2
	ld de,01101h            ; rb=0x11, g=0x01
	jp palette_set
...
title_sat_tmpl:
	...
	defb 04fh,070h
	defb 04fh,080h
; simon_land_test (seg1 0x7B6F): carry if the tile at (SimonX, SimonY+7) is solid.
simon_land_test:
	ld a,(simon_y)
	add a,007h              ; feet + 7
	ld e,a
...
simon_wall_right:
	...
	ld a,(simon_x)
	ld d,a
	ld bc,00802h            ; sample size
	xor a                   ; A=0 Simon-sized
	jr wall_right_go
...
whip_hit_apply:                 ; (0x7DDB) overlap: sfx, drop hittable, fodder vs meter
	ld a,sfx_hit
	call play_sound
	res 0,(ix+00eh)        ; one hit per rearm
	ld a,(ix+000h)         ; actor type
	sub actor_dracula      ; 0x11..0x17 = HP-bar
	cp 007h
	jr c,whip_hit_metered
...
whip_hp_dec:                    ; always -1 HP; leather skips extra
	...
	ld a,(ix+00dh)
	rla                    ; wrapped negative (bit7)?
	ret nc                 ; still alive
whip_kill_fodder:               ; (0x7E00)
	call award_kill_score
	jp actor_kill
whip_hit_metered:               ; (0x7E06) types 0x11..0x17; giant bat dual-use
	ld a,(ix+000h)
	cp actor_giant_bat     ; type 0x12: event-1 boss, or fodder on s16
	jr nz,whip_hit_hpbar
	ld a,(cell_event)
	and a
	jr z,whip_hit_fodder   ; no event: per-actor HP
whip_hit_hpbar:                 ; (0x7E13) 0xC418 -= table[weapon, type-0x11]
	call weapon_hit_damage
	ld a,(enemy_meter)
	and a
	jr z,boss_killed       ; bar already empty
	rla
	ret nc                 ; still positive
boss_killed:                    ; (0x7E1E) bar empty: flash, sfx, CE15, cull
	ld a,014h
	ld (0c445h),a          ; boss-death flash timer
	ld a,sfx_boss_clear
	call play_sound
	call award_kill_score
	ld a,001h
...
weapon_hit_damage:              ; (seg1 0x7E33)
	...
	cp equip_knife
	jr z,hpbar_dmg_lookup   ; knife -> weak table
	ld hl,hpbar_dmg_strong  ; chain/axe/cross -> 1.5x
hpbar_dmg_lookup:               ; (0x7E43) B = table[type-0x11]
	ld a,(ix+000h)          ; struck enemy type
	ld c,a
	sub actor_dracula       ; index = type - 0x11
	call ADD_HL_A
	ld b,(hl)
	ld a,c
...
hpbar_dmg_strong:              ; (0x7E67) chain/axe/cross (1.5x)
	defb 006h,00ch,00ch,006h,006h,006h,018h
actors_vs_simon:                ; (0x7E6E) C800 vs Simon
	ld a,(sapphire_ring)
	and a
	jr nz,actors_vs_simon_scan ; ring: skip i-frame / gem / already-hurt
	ld a,(simon_invuln)
	and a
	ret nz
	ld a,(blue_gem)
	and a
	ret nz                 ; invis: no contact
	ld a,(simon_action)
	cp act_hurt
	ret nc                 ; already hurt or dying
...
actor_simon_slot:               ; one C800 vs Simon
	...
	cp actor_reward
	jp z,actor_kill        ; sparkle drop, no contact dmg
	cp actor_orb           ; boss-clear orb (sprite 0x8F); not bonus id 22
	jr nz,simon_touch_enemy
	ld a,001h
	ld (orb_got),a         ; collected: drip-fill HP then advance stage
	jp actor_kill
simon_touch_enemy:              ; (0x7EB7) after orb/pickup: ring kill or contact
	ld a,(sapphire_ring)
	and a
	jr z,simon_contact_hurt
	call award_kill_score
	call actor_kill
	ld a,sfx_ring_kill
	jp play_sound
simon_contact_hurt:             ; (0x7EC8) arm act_hurt; Xvel sign -> hurt_facing
	ld a,act_hurt
	ld (simon_action),a
	xor a
	ld (simon_hurt_step),a
...
shot_simon_slot:
	...
	ld a,(sapphire_ring)
	and a
	jp nz,add_score_100    ; ring: destroy shot, +100, no hurt
	ld a,act_hurt
	ld (simon_action),a
	xor a
	ld (simon_hurt_step),a
...
;  Regen: tools/workbench/msx/regen-bank.sh 2 0x8000
; ===========================================================================
...
projectile_hit_apply:           ; (0x8053) overlap: sfx, then fodder HP or meter
	ld a,sfx_hit
	call play_sound
	ld a,(ix+000h)         ; actor type
	sub actor_dracula      ; 0x11..0x17 = HP-bar
	cp 007h
	jr c,projectile_hit_metered
projectile_hit_fodder:          ; (0x8061) ix+0D -= fodder_dmg_tbl[type-2]
	ld a,(iy+001h)         ; projectile type
	sub 002h               ; 2=knife .. 5=holy -> 0..3
	ld hl,fodder_dmg_tbl
	call ADD_HL_A
	ld b,(hl)
	ld a,(ix+00dh)         ; actor HP
	sub b
	ld (ix+00dh),a
	jr z,projectile_kill_fodder
	jp p,projectile_after_hit ; still alive (signed)
projectile_kill_fodder:         ; (0x8079)
	call award_kill_score
	call actor_kill
projectile_after_hit:           ; (0x807F) knife despawns; axe/cross/holy keep flying
	ld a,(iy+001h)
	cp 002h                ; knife?
	push iy
	pop hl
	jp z,projectile_clear_hl
	res 0,(ix+00eh)        ; keep projectile, drop hittable until rearm
	ret
projectile_hit_metered:         ; (0x808F) types 0x11..0x17; giant bat dual-use
	ld a,(ix+000h)
	cp actor_giant_bat     ; type 0x12: event-1 boss, or fodder on s16
	jr nz,projectile_hit_hpbar
	ld a,(cell_event)
	and a
...
projectile_hit_hpbar:           ; (0x809C) 0xC418 -= table[weapon, type-0x11]
	...
	rla
	jr nc,projectile_after_hit ; still positive
projectile_meter_empty:         ; (0x80A8) bar empty: boss_killed, then after_hit
	call boss_killed
	jr projectile_after_hit
...
vendor_vs_attack:              ; (0x80EC) one vendor slot HL vs whip then proj
	ld a,(hl)
	rla
	ret nc                 ; bit7 clear = inactive
	ld a,(weapon_id)
	cp equip_knife
	jr nc,vendor_try_proj  ; thrown: skip whip box
	ld a,(simon_whip)
	cp 003h                ; hit frame only
	jr nz,vendor_try_proj
	push hl
	call obj_vs_whip_lo
	pop hl
	jr c,vendor_struck
vendor_try_proj:                ; (0x8104)
	push hl
	call obj_vs_proj_lo
	pop hl
	ret nc
	ld a,(iy+001h)
	cp equip_knife
	jr nz,vendor_struck    ; knife is consumed
	push hl
	push iy
	pop hl
	call projectile_clear_hl
	pop hl
vendor_struck:                  ; (0x8119) +3 hit counter
	inc l
	inc l
	inc l
	inc (hl)
...
hourglass_hit_test:             ; (0x813C)
	ld a,(weapon_id)
	cp equip_knife
	jr nc,hourglass_try_proj
	ld a,(simon_whip)
	cp 003h                ; hit frame only
	jr nz,hourglass_try_proj
	push hl
	call obj_vs_whip_hi
	pop hl
	jr c,hourglass_struck
hourglass_try_proj:             ; (0x8151) any projectile; always consumed
	push hl
	call obj_vs_proj_hi
	pop hl
	...
	call projectile_clear_hl
	pop hl
hourglass_struck:               ; (0x8160) +3=2 -> pickup_idle / hourglass_tip
	inc l
	inc l
	inc l
	ld (hl),002h
...
; the kill score - see actor_value_tbl below).  Then:
...
;     contact = 2 * (actor_value_tbl odd byte).  Runtime-confirmed: zombie(t01) odd 1 -> 2,
;     dog(t05) odd 3 -> 6 (0x1E->0x18).  Then jp damage_health (0xC415 -= B).
hurt_simon_contact:
	ld a,(ix+000h)
	dec a
	add a,a
	ld hl,actor_value_tbl
	call ADD_HL_A
	inc hl                 ; odd byte = base contact damage
	ld b,(hl)
	ld a,(0c701h)
	bit 4,a                ; red shield (id 3)?
	jr z,contact_dmg_full  ; no shield -> doubled
	ld a,(simon_x)
	sub (ix+005h)          ; CY if actor is to Simon's right
	ld a,(simon_facing)
	jr nc,contact_dmg_left ; actor to the left
	and a
	jr z,contact_dmg_block ; facing right + actor right = face-on
	jr contact_dmg_full    ; facing left = backstab
contact_dmg_left:               ; (0x8197) actor left of Simon
	and a
	jr nz,contact_dmg_block ; facing left + actor left = face-on
contact_dmg_full:               ; (0x819A) unshielded / backstab: B *= 2
	ld a,b
	add a,a
	ld b,a
	jr contact_dmg_apply
contact_dmg_block:              ; (0x819F) face-on red shield: spend a charge
	ld hl,0c441h
	dec (hl)
	jr nz,contact_dmg_apply
	ld hl,0c701h
	res 4,(hl)             ; charges gone -> drop the HUD icon
	push bc
	call hud_bonus_refresh
	pop bc
contact_dmg_apply:              ; (0x81AF)
	jp damage_health       ; 0xC415 -= B
; award_kill_score (seg2 0x81B2): give points for killing the actor in IX.
; Looks up the per-type hundreds value D from table actor_value_tbl[(type-1)] (E=0 low pair),
; then picks the high pair C by type (0x11 -> 3, 0x17 -> 5, else 0) and calls
; add_score with C:D:E.
; award_kill_score (seg2 0x81B2): add this actor's kill points.  Even byte of
; actor_value_tbl[type-1] is BCD hundreds -> D (E=0).  C is the high pair:
; Dracula 30000, type 0x17 50000, everyone else 0.  Then add_score.
award_kill_score:
	ld a,(ix+000h)          ; type
	ld b,a                  ; keep for the 0x11/0x17 tests
	dec a                   ; table is 1-based
	add a,a                 ; 2 bytes/entry
	ld hl,actor_value_tbl
	call ADD_HL_A           ; HL -> even (score/100) byte
	ld e,000h               ; low pair = 00
	ld d,(hl)               ; D = hundreds pair for this enemy type
	ld a,b
	cp actor_dracula
	ld c,003h               ; type 0x11: 30000
	jp z,add_score
	ld c,005h               ; type 0x17: 50000
	cp 017h
	jp z,add_score
	ld c,000h               ; fodder: just the table hundreds
	jp add_score
; actor_value_tbl (0x81D5) - per-actor-type table, 2 bytes/entry, indexed by (type - 1):
...
actor_value_tbl:
	...
	defb 000h,000h, 000h,000h
; add_score_100 (seg2 0x8231): +100 for destroying a shot (C=0 high pair).
add_score_100:
	ld de,00100h            ; 100
	jp add_score_c0
pickup_vs_simon:               ; (0x8237) C500 slot HL vs Simon (Y+10, X+8, 10x8)
	...
	ld hl,01008h
	jp overlap_simon
; proj_overlap_simon (seg2 0x8247): C450/C460 box (16x8 at +5,+4) vs Simon
; (boomerang catch / holy-water flame).
proj_overlap_simon:
	ld d,(ix+005h)          ; X
	ld e,(ix+004h)          ; Y
	ld hl,01008h            ; 16 x 8
...
; hit_class_c800 (seg2 0x82BF): C800 box class in A (0..6 after `dec a`),
; B=pixel X, C=pixel Y.  Table is 1-based classes 1-7 (fodder 5x24 .. grim 8x48).
hit_class_c800:
	ld a,(ix+000h)          ; type
	dec a                   ; 1-based table
	...
	call ADD_HL_A
	ld a,(hl)               ; class 1..7
	dec a                   ; DISPATCH index 0..6
	ld b,(ix+005h)          ; pixel X
	ld c,(ix+003h)          ; pixel Y
	ret
...
; hit_class_shot (seg2 0x8302): D700 box class in A (0..2, no extra dec),
; B=pixel X, C=pixel Y.  0 = 3x6, 1 = 6x6, 2 = 6x12 (bone/axe/sickle).
hit_class_shot:
	ld a,(ix+000h)          ; shot type
	dec a                   ; 1-based table
...
box_10x08_proj:                ; 0x83BD
	ld hl,01008h
	jp overlap_projectile
; obj_xy_20_10 (seg2 0x83C3): E=Y+0x20, D=X+0x10 from compact slot HL (+1 Y, +2 X).
obj_xy_20_10:
	inc l                   ; +1 Y
	ld a,(hl)
	add a,020h
	...
	ld d,a
	ret
; obj_xy_10_8 (seg2 0x83CE): E=Y+0x10, D=X+0x08 from compact slot HL (+1 Y, +2 X).
obj_xy_10_8:
	inc l                   ; +1 Y
	ld a,(hl)
	add a,010h
...
candle_vs_whip:                ; (0x8457) C470 slot HL vs whip
	...
	ld hl,01008h
	jp overlap_whip
candle_vs_proj:                 ; (0x8467) C470 vs C450/C460; type<2 8×16 else 16×32
	ld a,(ix+004h)
	cp 002h                ; 0/1 candle, 2+ block
	ld hl,01008h           ; H=16 W=8
	jr c,candle_proj_test
	ld hl,02010h           ; H=32 W=16
candle_proj_test:               ; (0x8474) DE = slot XY + box
	ld a,(ix+001h)
	add a,h
	ld e,a
	ld a,(ix+002h)
...
overlap_simon_dx:               ; abs dx vs box width L
	...
	ld b,a
	dec a
	cp 002h                ; jump/crouch: shorter
	ld a,012h
	jr c,overlap_simon_ht
	ld a,01ah              ; grounded/stairs/fall/hurt
overlap_simon_ht:               ; (0x849D)
	add a,h
	ld h,a
	ld a,b
	dec a                  ; Z if jumping
	ld a,(simon_y)
	jr nz,overlap_simon_y
	sub 008h               ; jump: lift the box
overlap_simon_y:                ; (0x84A8)
	sub 002h
	sub e
	add a,c
	cp h
...
overlap_whip:                  ; (0x84AE) DE=actor XY, HL=box; vs whip (facing + C416)
	ld a,(weapon_id)
	dec a                  ; Z if chain
	ld a,00ch              ; leather / thrown
	jr nz,overlap_whip_w
	ld a,010h              ; chain: longer
overlap_whip_w:                 ; (0x84B8)
	add a,l
	ld l,a
	call whip_reach_x
	jr nc,overlap_whip_x
	xor a                  ; tip wrapped off-screen
	ret
...
overlap_whip_dx:                ; abs tip-X vs actor X
	...
	cp act_crouch
	ld b,012h              ; stand
	jr nz,overlap_whip_y
	ld b,00ah              ; crouch: shorter
overlap_whip_y:                 ; (0x84D3)
	ld c,h
	ld a,004h
	add a,h
	ld h,a
	...
	cp h
	ret
whip_reach_x:                   ; (0x84E0) A = whip tip X; CY = wrapped
	ld a,(weapon_id)
	dec a
	ld b,014h              ; leather / thrown
	jr nz,whip_reach_len
	ld b,018h              ; chain
whip_reach_len:                 ; (0x84EA)
	ld a,(simon_facing)
	and a
	ld a,(simon_x)
	jr z,whip_reach_right
	sub b                  ; facing left: X-reach
	ret
whip_reach_right:               ; (0x84F5) facing right: X+reach
	add a,b
	ret
overlap_projectile:            ; (0x84F7) DE=actor XY, HL=box; vs C450 then C460
	...
	call proj_slot_overlap
	ret
; proj_slot_overlap (seg2 0x8507): IY projectile vs box DE=(X,Y) HL=(W,H).
; NC = miss.  Knife uses height 6, other shots 0x0C.
proj_slot_overlap:
	ld a,006h               ; skip to box W at L+6
	add a,l
	ld l,a                  ; L = width
	ld a,(iy+005h)          ; projectile X
	sub d                   ; vs box X
	jr nc,proj_slot_dx
	neg                     ; abs dx
proj_slot_dx:                   ; abs projectile X vs box
	cp l
	ret nc                  ; |dx| >= width: miss
	ld a,(iy+001h)          ; projectile kind
	cp equip_knife
	ld b,00ch               ; axe/cross/holy height
	jr nz,proj_slot_ht
	ld b,006h               ; knife: shorter
proj_slot_ht:                   ; (0x8520)
	ld c,h                  ; C = box height
	ld a,b                  ; shot height
	add a,h
	ld h,a                  ; H = boxH + shotH
	ld a,(iy+004h)          ; projectile Y
	sub e                   ; vs box Y
	add a,c                 ; + boxH
	cp h                    ; in [0, boxH+shotH)
	ret
...
platform_stand_test:
	ld a,(simon_action)
	cp act_fall
	jr nz,platform_stand_scan
	ld a,(simon_arc)
	cp 003h
	ret c                  ; still rising -> don't land on anything
platform_stand_scan:            ; (0x8538)
	ld hl,platform_slots
...
platform_overlap:
	...
	sub (hl)               ; - platform Y (+1)
	cp 008h
	jr nc,platform_miss    ; Y miss: not resting on the deck
	ld b,007h
	inc hl
	ld a,(simon_x)          ; Simon X
	sub b
	...
	cp 020h
	jr c,platform_hit      ; within the 32px deck
	ld a,(simon_x)
	add a,b
	sub (hl)               ; right foot
	cp 020h
	jr c,platform_hit
platform_miss:                  ; (0x8575) even id -> D=0; odd -> E=0
	ld a,c
	rra
	jr c,platform_miss_e
	ld d,000h
	ret
platform_miss_e:                ; (0x857C)
	ld e,000h
	ret
platform_hit:                   ; (0x857F) even id -> D=id; odd -> E=id
	ld a,c
	rra
	jr c,platform_hit_e
	ld d,c
	ret
platform_hit_e:                 ; (0x8585)
	ld e,c
	ret
...
door_proximity:
	ld a,(simon_facing)
	and a
	jr z,door_prox_right    ; facing 0 = right: X+8
	ld a,c
	sub 008h                ; facing left: X-8
	ld c,a
	jr door_prox_test
door_prox_right:                ; (0x8593)
	ld a,c
	add a,008h
	ld c,a
door_prox_test:                 ; (0x8597) Y window 0x38, X window 8
	dec b
	dec b                   ; Y-2
	ld a,(door_y)           ; door Y
	sub 008h
...
; Skipped while Simon is already dying (0xC420==6) or during
; simon_invuln / blue_gem timers.
hurt_simon_spikes:
	ld a,(simon_action)
	cp act_dying
...
brazier_tick:
	...
	ld a,b                  ; old state
	cp 001h
	jr nz,candle_anim      ; already stamped
	ex af,af'
	push af
	push de
	...
	pop af
	cp 002h
	jp c,candle_flame      ; kind 0/1: candle flame
	jp block_stamp         ; kind 2/3: overlay brick tiles
candle_anim:                    ; (0x86BA) every 4 frames; blocks stay stamped
	ld a,(hl)
	and 003h
	ret nz
	ex af,af'              ; kind
...
candle_flame:                   ; (0x86C2) kind 0 -> frames 0/1; kind 1 -> 2/3
	or a
	ld a,000h
	jr z,candle_frame
	ld a,002h
candle_frame:                   ; (0x86C9) +6 bit3 toggles the pair
	bit 3,(hl)
	jr z,candle_flame_go
	inc a
candle_flame_go:                ; (0x86CE)
	jp candle_blit
...
block_tileset:                  ; (0x86DE)
	ld a,b
	ld bc,01002h           ; 16x16, 2x2 tiles
	cp 002h
	jr z,block_stamp_go
	ld bc,02004h           ; 32x32, 4x4
	ld a,004h
	call ADD_HL_A          ; skip leading 2x2 bytes
block_stamp_go:                 ; (0x86EE) blit VRAM then D100
	push bc
	push de
	ld b,c
	push bc
...
vdp_box_white:
	...
	jp vdp_box
; candle_outlines_if (seg2 0x870E): if C702 bit0 (candle bonus), walk C470
; and outline live blocks in white.
candle_outlines_if:
	ld a,(0c702h)           ; bible/candle flags
	rra                     ; bit0 = candle outlines
	ret nc
candle_outlines:
	ld bc,00800h            ; B=8 slots
	ld hl,scenery_slots
...
block_save_under:
	cp 003h
	ld hl,0e4a0h
	jr z,map_save_4x4      ; kind 3: 4x4 stash
	ld hl,0e480h           ; 2x2 stash
map_save_2x2:                   ; (0x874B) dest HL, slot C * 4; also pickups -> E520
	push hl
	push bc
	call map_cell_at
	pop bc
	...
	ld bc,00202h
	jp map_copy_rows
map_save_4x4:                   ; (0x875E) dest HL, slot C * 16; also vendor -> E580
	push hl
	push bc
	call map_cell_at
	pop bc
	...
	ld bc,00404h
	jp map_copy_rows
map_copy_rows:                  ; (0x8773) B rows of C bytes; HL += 32 per row
	push bc
	ld b,000h
	push hl
	ldir
...
brazier_destroyed:
	...
	cp 002h
	jp nc,block_break      ; kind 2/3: bricks
	call block_restore_vram_2x2
	call scenery_clear_rec
	jp scenery_drop
block_break:                    ; (0x87D9) sfx, then restore 2x2 or 4x4
	push bc
	push de
	push bc
	ld c,a
	ld a,sfx_block_break
	call play_sound
	ld a,c
	pop bc
	cp 002h
	jr z,block_break_2x2
	call block_restore_vram_4x4
	call block_restore_map_4x4
	jr scenery_break_result
block_break_2x2:                ; (0x87F0)
	call block_restore_vram_2x2
	call block_restore_map_2x2
scenery_break_result:          ; (0x87F6) after tile restore: drop / key / reveal
	ld a,(ix+005h)         ; bonus id
	cp 01fh
	jr z,scenery_reveal            ; 0x1F = reveal (third scenery byte)
	cp item_white_key
	jr z,scenery_white_key
	call scenery_clear_rec
	pop de
	pop bc
	ld a,b
	or a
	jr z,break_chip_go     ; empty: chips only
	ld a,(ix+004h)
	cp 003h
	jr nz,scenery_drop_xy
	ld a,e
	add a,010h             ; 4x4 block: drop at Y+16
	ld e,a
scenery_drop_xy:                ; (0x8815)
	call drop_spawn
break_chip_go:                  ; (0x8818)
	jp break_chip_spawn
...
;  otherwise -> chest at Y+16 (bottom half of the 4x4) via pickup_spawn_chest. Same
;  Y+16 as a kind-3 whip drop; scenery Y is the top of the stamp.
scenery_reveal:                ; (0x881B)
	pop de
	pop bc
	...
	cp 0c0h
	ld a,b
	jr z,reveal_vendor     ; vendor: keep stamp Y
	and 01fh
	ld b,a
	ld a,e
	add a,010h             ; chest: Y+16
	ld e,a
	jp pickup_spawn_chest
reveal_vendor:                  ; (0x8838) bits5-2 offer slot, bits1-0 facing
	ld c,a
	and 03ch
	rrca
	rrca
	...
	ld c,a
	jp vendor_spawn
scenery_white_key:              ; (0x8845) bonus 0x18: E000 pos -> white_key_spawn
	ld b,a
	pop de
	pop bc
	ld h,(ix+007h)
	ld l,(ix+008h)
	jp white_key_spawn
; block_restore_vram_2x2 (seg2 0x8851): blit E480 2x2 stash (slot C) at DE.
block_restore_vram_2x2:
	ld hl,0e480h            ; 2x2 nametable stash
...
tiles_blit_4x4:
	...
	pop bc
	ret
; scenery_clear_rec (seg2 0x887B): zero the E000 pos byte via ix+7/+8.
scenery_clear_rec:
	ld h,(ix+007h)          ; E000 pos hi
	ld l,(ix+008h)          ; E000 pos lo
	ld (hl),000h            ; record gone
	ret
...
tiles_to_map_4x4:                     ; also: 4x4 map restore from caller HL (vendor 0xE580)
	...
	pop bc
	ret
; tiles_to_map (seg2 0x88BE): B rows of C nametable bytes HL -> DE, then DE += 32.
tiles_to_map:
	push bc
	ld b,000h               ; BC = C (row width)
	push de
	ldir                    ; one nametable row
	pop de
	pop bc
	ld a,020h               ; 32 tiles/row
	call ADD_DE_A
	djnz tiles_to_map
	ret
...
break_chip_sat_cell:
	...
	call chip_fill16
	ld a,04ch
; chip_fill16 (seg2 0x8979): fill 16 colour bytes at HL with A (break-chip SAT).
chip_fill16:
	ld b,010h               ; 16 lines
chip_fill16_loop:
	ld (hl),a               ; colour
...
drop_spawn:                        ; (seg2 0x89EA) flame+C500, or slime directly to scenery_drop_slot
	...
	call pickup_slot_alloc
	ret nz
; pickup_slot_write (seg2 0x89F7): C500 live 0x83, XY from DE, bonus B, +5=FF.
pickup_slot_write:
...
pickup_link_e000:                  ; (seg2 0x8A12) +0E/+0F = E000 pos from C70D
	...
	ld (hl),e
	ret
pickup_spawn_chest:            ; (0x8A1A) C500 chest container; contents in +0x0D
	ld (0c70dh),hl
	call pickup_slot_alloc
	ret nz
	push bc
	...
	inc l
	jr pickup_link_e000
; drop_own_weapon_heart (seg2 0x8A30): if B is the equipped subweapon pickup,
; swap it for a small heart (cannot drop your own whip/subweapon).
drop_own_weapon_heart:
	ld a,b                  ; drop bonus id
	cp item_chain_whip
...
; Z + HL -> slot if found, else NZ.
pickup_slot_alloc:
	push bc
	ld hl,pickup_slots      ; C500
	ld b,008h               ; 8 floor-item slots
pickup_slot_scan:
	ld a,(hl)               ; type
	or a
...
pickup_clear_e000:                 ; (seg2 0x8C12) zero the linked scenery pos
	...
	ld (hl),000h
	ret
chest_open:                    ; (0x8C1B) spill contents; spend yellow key/lockpick
...
hourglass_tip:                 ; (0x8C4B) whip: id 10 -> 11; second hit starts despawn
	ld (ix+003h),000h
	ld a,(ix+004h)
...
; bonus_icon_blit (seg2 0x8CC8): 16x16 LMMM from page-0 atlas.  Ids 0-15 at
; Y=0x50, 17+ at Y=0x60 (A -= 16).  Dest DE, logic 0x48 (page1->0, colour-0 skip).
bonus_icon_blit:
	ld l,050h               ; atlas Y for ids 0-15
	cp 010h
	jr c,atlas_lmmm16
	sub 010h                ; ids 16+
	ld l,060h               ; ids 17+ at Y=0x60
atlas_lmmm16:                   ; (0x8CD2) 16x16 LMMM from (A*16, L) page 0
	add a,a
	add a,a
	add a,a
	add a,a                 ; A*16 = atlas X
	ld h,a
	ld bc,01010h            ; 16x16
	ld a,048h               ; LMMM page1->0
	jp vdp_lmmm
pickup_save_under:                 ; (seg2 0x8CDF) nametable 2x2 under pickup -> E520
	...
	ld hl,0e520h
	jp map_save_2x2
; pickup_restore_under (seg2 0x8CED): put the saved 2x2 nametable (E520) back
; at the pickup cell, blit it, then redraw vendor/door/candle overlays.
pickup_restore_under:
	push ix
	push de
	push de
	ld c,(ix+007h)          ; saved map column
	ld hl,0e520h            ; saved 2x2
	push bc
	push hl
	call map_cell_at        ; HL -> nametable cell
	ld de,0e800h            ; scratch
	ld bc,00202h            ; 2x2
	call map_copy_rows      ; current tiles -> E800
	pop hl
	pop bc
	push bc
	push hl
...
;  25-entry word table collect_bonus_tbl at 0x8D45 (index = A-1; A>=0x1A falls through to collect_weapon):
...
collect_bonus:
	...
	push hl
collect_bonus_apply:
	ld (bonus_id),a          ; latch last-collected bonus id
	call pickup_popup_show
	cp item_chain_whip     ; id>=0x1A -> collect_weapon (holy water 0x1E too)
	jr nc,collect_weapon
	dec a
	call DISPATCH_A
collect_bonus_tbl:             ; (seg2 0x8D45) word[id-1]; id>=0x1A -> collect_weapon
...
; frame and no new enemies spawn. Duration: 0xF0 (~4s) if bonus_flags bit2
; (tipped hourglass), else 0x96 (~2.5s). Same bit lengthens blue gem, sapphire
; ring, and hourglass freeze. Counts down in timers_tick. Existing C800 actors
; stay. Weapon pickups fall through into this same code (brief no-spawn window).
bonus_rosary:
	ld a,(bonus_flags)     ; bit2 = tipped hourglass
	and 004h
...
rosary_arm:                     ; (0x8D8E)
	ld (rosary_timer),a
bonus_ret_collect:              ; (0x8D91) A = sfx_collect
	ld a,sfx_collect
	ret
bonus_holy_water:              ; id 0x1E (0x8D94): C701 bit3; jump+LEFT/RIGHT, 5 hearts
	ld b,008h
bonus_grant_c701:               ; (0x8D96) C701 |= B, then collect sfx
	call inv_or_c701
	jr bonus_ret_collect
...
bonus_blue_gem:                ; id 8 (0x8DD4): invis; sprite flash white
	ld a,(bonus_flags)
	and 004h               ; tipped -> longer
	ld a,0f0h              ; 240 frames ~4s
	jr nz,blue_gem_arm
	ld a,096h              ; 150 frames ~2.5s
blue_gem_arm:                   ; (0x8DDF)
	ld (blue_gem),a        ; skip contact + projectile hits while nonzero
	ld a,sfx_blue_gem
	ret
...
bonus_sapphire_ring:           ; id 9 (0x8DEC): sprite flash red; touch-kills
	...
	and 004h
	ld a,0f0h
	jr nz,sapphire_ring_arm
	ld a,096h
sapphire_ring_arm:              ; (0x8DF7)
	ld (sapphire_ring),a
	jr bonus_done
bonus_tipped_hourglass:        ; id 11 (0x8DFC): bonus_flags bit2, 1.5x timed bonuses
	ld hl,bonus_flags      ; whip the hourglass pickup once to get this
	set 2,(hl)
	jr bonus_done
bonus_boots:                   ; id 12 (0x8E03): bonus_flags bit3 faster walk
	ld hl,bonus_flags
	set 3,(hl)
	jr bonus_done
bonus_wings:                   ; id 13 (0x8E0A): bonus_flags bit4 higher jump
	ld hl,bonus_flags
	set 4,(hl)
	jr bonus_done
bonus_done:                     ; (0x8E11) jr-reach collect sfx
	jp bonus_ret_collect
...
; returns with no effect.  World chests never get here (chest_open opens them).
bonus_slime:                   ; id 21 (0x8E1B): fake candle drop; hatches if left
bonus_chest:                   ; id 25: treasure-chest container (see pickup_spawn_chest)
	pop hl
	ret
...
bonus_white_bible:             ; id 17 (0x8E2D): C702 bit7, vendor price halved
	...
	ld b,080h
bonus_grant_c702:               ; (0x8E34) C702 |= B, then collect sfx
	call inv_or_c702
	ld a,sfx_collect
	ret
bonus_map:                     ; id 15 (0x8E3A): bonus_flags bit6, C701 bit7, C70F=3
	ld hl,bonus_flags
	set 6,(hl)
	ld a,003h
...
bonus_white_key:               ; id 24 (0x8E67): C701 bit0 (stage-exit door)
	...
	pop hl
	ret
; inv_or_c701 (seg2 0x8E74): OR bit-mask B into C701 (keys / shields / map).
; inv_or_c702 does the same for C702 (bible flags).
inv_or_c701:
	ld hl,0c701h            ; keys/shields/map bits
	jr inv_or_apply
inv_or_c702:
	ld hl,0c702h            ; bible flags
inv_or_apply:                   ; (0x8E7C)
	ld a,b                  ; bit to set
	or (hl)                 ; merge
	ld (hl),a
...
hud_bonus_hmmm:                    ; (seg2 0x8EB8) 16x16 from page 1
	jp vram_hmmm16
; hud_keys_refresh (seg2 0x8EBB): chest-key lamp then white-key lamp.
hud_keys_refresh:
	call hud_chest_key_icon
	jp hud_white_key_icon
; hud_white_key_icon (seg2 0x8EC1): HUD lamp at (0xA4,0x0C).  Leather if
; C701 bit0 clear, else bonus 0x18 (white key).
hud_white_key_icon:
	ld de,0a40ch            ; HUD dest
	ld a,(0c701h)
	and 001h                ; white-key bit
	jp z,hud_leather_icon
	ld a,018h               ; item_white_key
	jr hud_bonus_tile
; hud_chest_key_icon (seg2 0x8ED0): HUD lamp at (0x94,0x0C).  Leather whip if
; C701 has neither yellow-key (bit1) nor lockpick (bit2), or C700==0; else
; lockpick vs yellow-key from bit2.
hud_chest_key_icon:
	ld de,0940ch            ; HUD dest (X,Y)
	ld a,(0c701h)           ; inventory bits
	ld b,a
	and 006h                ; bit1 yellow key / bit2 lockpick
	jp z,hud_leather_icon   ; neither -> leather
	ld a,(0c700h)           ; key/lockpick charge
	or a
	jp z,hud_leather_icon   ; spent -> leather
	bit 2,b                 ; C701 bit2 = lockpick
	ld a,item_lockpick
	jr nz,hud_chest_key_id
	ld a,item_yellow_key
hud_chest_key_id:               ; (seg2 0x8EEB)
	jr hud_bonus_tile       ; blit that bonus tile
hud_bonus_refresh:
...
; EVERY pickup (via 0x8F2A), so 0xC5E5/0xC5E6 are generic - NOT rosary-specific.
	...
	pop ix
	ret
; hud_bonus_clear (seg2 0x8F51): HMMV blank 64x16 at (0xB8,0x0C) page 0.
hud_bonus_clear:
	ld hl,0b80ch            ; dest (X,Y)
	ld bc,04010h            ; 64 x 16
	xor a                   ; colour 0
	ld d,a                  ; page 0
	jp vdp_hmmv
...
platform_tbl:                      ; (seg2 0x9073) {stage,room,n} + n x {Y,X,step,span}; 0xFF end
	...
	defb 0ffh
; platform_tick (seg2 0x90A2): 2 x C598 moving platforms.  Live slots move
; then rebuild SAT.  Simon ride is C439, applied in platform_carry_simon.
platform_tick:
	ld hl,platform_slots
	ld b,002h               ; 2 slots
platform_slot:
...
; platform_fill16 (seg2 0x9112): fill 16 colour bytes at HL with A (one
; 16x16 sprite's SAT lines).
platform_fill16:
	ld b,010h               ; 16 lines
...
door_anim_hold:                    ; (seg2 0x916F) keep C5AC = 3 (open)
	...
	ret
; door_begin_open (seg2 0x9175): if door_state==1, set 0xFF and start the blit.
door_begin_open:
	ld hl,door_state
	ld a,(hl)
	dec a
	ret nz                  ; only when C5AC == 1 (armed)
...
vendor_offer_done:                 ; (seg2 0x91C2) A = matching slot 1/2, or 0
	...
	exx
	ret
; vendor_tick (seg2 0x91C5): 2 x C5B5/C5C5 vendor slots, stride 0x10.
vendor_tick:
	ld hl,0c5b5h            ; first vendor object
	ld bc,00200h            ; B=2, C=slot index
vendor_tick_slot:
	push bc
	push hl
	push hl
...
vendor_whip_leave:                 ; (seg2 0x921C) force outcome 6 (leave)
	...
	ld (0c70bh),a
	jr vendor_flash_arm
; vendor_slot_ptr (seg2 0x9228): HL -> DE00 offer byte for this vendor (ix+9).
vendor_slot_ptr:
	ld a,(ix+009h)          ; compact slot 0/1
	call vendor_de00_slot
	inc hl                  ; skip offer id -> whip-count / flags
	ret
...
vendor_draw_idle:
	ld a,003h
; vendor_draw (seg2 0x9265): A = C70B 0..4 -> SX=A*32, SY=0xA0, 32x32 LMMM
; page-1 -> page-0 (colour 0 skip) at DE=(X,Y).
vendor_draw:
	rrca                    ; A*32 via three rotates into H
	rrca
	rrca
	ld h,a                  ; SX
	ld l,0a0h               ; SY = 0xA0 (page-1 vendor cache)
	ld a,048h               ; LMMM page1->0, colour-0 skip
	ld bc,02020h            ; 32x32
	jp vdp_lmmm
; vendor_redraw_all (0x9273): idle-blit every occupied C5B5/C5C5 slot
; (after a screen restore: pickup, F2 map close).
vendor_redraw_all:
	ld hl,0c5b5h            ; first vendor slot
	ld bc,00200h            ; B=2 slots, C=index
vendor_redraw_slot:
	push bc
	push hl
	push hl
...
; (sfx 0x19) and draw the price/item bubble (vendor_offer_draw).  Called from the resident
; vendor state machine (seg0 vendor_begin) while seg2 is paged in.
vendor_make_offer:
	call vendor_set_offer_item
	ld a,014h               ; 20 ticks
	ld (0c706h),a           ; offer timer; decremented in vendor_purchase_tick
	ld a,sfx_vendor_offer
	call play_sound
	jp vendor_offer_draw
vendor_offer_draw:             ; (0x939E) HMMM backup, panel, glyphs, price, icon
	ld a,(0c703h)
	ld hl,0c5b5h
	add a,a
	add a,a
...
; vendor_de00_this (seg2 0x9453): HL -> DE00 row for C703 (this vendor).
vendor_de00_this:
	ld a,(0c703h)           ; latched compact id
; vendor_de00_slot (seg2 0x9456): HL -> DE00 row for slot A (0/1).
; Courtyard skips the stage offset table; room*4 then +2 if A!=0.
vendor_de00_slot:
	push af                 ; slot 0/1
	ld bc,00000h            ; courtyard hub ofs = 0
	ld a,(stage)
	or a
vendor_de00_chk:
...
vendor_de00_base:                  ; (seg2 0x9468) DE00 + hub ofs + room*4
	...
	pop af                  ; slot
	or a
	ret z                   ; slot 0: first pair
	inc hl
	inc hl                  ; slot 1: second pair
	ret
vendor_de00_ofs:
...
vendor_price_blit:
	...
	ld b,001h
	jp hud_bcd_draw
; vendor_glyphs (seg2 0x94A6): 0xFF-terminated HUD string at HL via vendor_bubble_xy.
vendor_glyphs:
	call vendor_bubble_xy
vendor_glyph_loop:
	ld a,(hl)
...
; vendor_bubble_xy (seg2 0x94B6): DE = C704 bubble origin + (B,C) pixel offset.
vendor_bubble_xy:
	ld de,(0c704h)          ; bubble origin (X,Y)
	ld a,d
	add a,b                 ; X += B
	ld d,a
	ld a,e
...
vendor_offer_wipe:
	push bc
	ld hl,0b080h            ; page-1 stash of the playfield under the bubble
	ld de,(0c704h)          ; bubble origin
	ld bc,05020h            ; 80 x 32
	ld a,001h               ; HMMM page1->0
	call vdp_hmmm
	pop bc
	ret
...
vendor_read_buttons:
	...
	and (hl)
	ret
; read_kbd_matrix_bit (seg2 0x9552): SNSMAT row A, inverted bit0 -> 0/1.
read_kbd_matrix_bit:
	call SNSMAT
	cpl                     ; 0 = pressed on MSX
	and 001h                ; bit0 only
	ret
...
shot_sat_build:                    ; (seg2 0x9917) shot shape streams -> actor SAT
	ld ix,shot_slots
	ld b,008h               ; 8 D700 slots
	jr sat_build_loop
; c800_sat_build (seg2 0x991F): 7 x C800 shape streams -> actor SAT via
; actor_sat_build.  Shared loop with shot_sat_build (B=8).
c800_sat_build:
	ld ix,actor_slots
	ld b,007h               ; 7 C800 slots
...
actor_freeze_check:
	ld a,(0d010h)           ; freeze latch (hourglass / whip)
	and a
	ret z                   ; not frozen
	ld a,(ix+07eh)          ; skip-freeze flag
	and a
	ret z                   ; flames/pickups keep animating
	scf                     ; skip type tick + integrate
	ret
...
tick_nop_seg2:
	ret
; actors_rearm_hittable (seg2 0x99A6): restore hittable (bit0) on every C800
; then D700 slot whose +0E still has bit2.  A hit does `res 0`; this runs
; from the flame flicker so the next frame can connect again.
actors_rearm_hittable:
	ld de,00080h            ; slot stride
	ld hl,0c80eh            ; C800+0E (7 actors)
	ld b,007h
	call hittable_rearm_scan
	ld hl,0d70eh            ; D700+0E (8 shots)
	ld b,008h
; hittable_rearm_scan (0x99B2): if (HL) bit2, set bit0.  B slots, stride DE.
hittable_rearm_scan:
	bit 2,(hl)              ; rearm-enable still set?
	jr z,hittable_next      ; no -> leave bit0 alone
	set 0,(hl)              ; hittable this frame
hittable_next:
	add hl,de               ; next slot
...
actor_cull_offscreen:
	ld a,(ix+003h)          ; pixel Y
	cp 0e4h
	jr nc,actor_free        ; Y off the bottom
	ld a,(ix+005h)          ; pixel X
	cp 0f1h
	jr nc,actor_free        ; X off the right
	cp 007h
	ret nc                  ; X in 0x07..0xF0: keep
...
actor_hide_sat:                 ; Y=0xE0 for each claimed SAT cell
	...
	ret
; shot_death_flame (seg2 0x9A21): free this D700 slot, then spawn kind 0xFF
; (death flame, type 12) at the same pixel with vel 0.  Clears drop-gate +1F
; and freeze-respect +7E so the flame is not a pickup and keeps ticking.
shot_death_flame:
	call actor_free
	ld c,(ix+003h)          ; Y
	ld b,(ix+005h)          ; X
	...
	ld e,l
	ld d,h                  ; Xvel 0
	ld a,0ffh               ; kind = death flame
	push ix
	call shot_spawn
	ld (ix+01fh),000h       ; no drop
	ld (ix+07eh),000h       ; ignore D010 freeze
	pop ix
	ret
actor_cull:                    ; (0x9A41) CFFF=1: free, skip random drops
	ld a,001h
	jr actor_kill_flag
; actor_kill (seg2 0x9A45): CFFF=0 then actor_kill_go (whip/touch; may drop).
actor_kill:
	xor a                   ; allow drops
...
kill_drop_spawn:
	...
	scf
	ret
; kill_drop_pos (seg2 0x9B29): DE = (actor X, Y - kill_drop_dy[type]).
; Falls into the table; type 0's 0xC9 is also the RET that exits.
kill_drop_pos:
	ld a,(0cff0h)           ; stashed actor type
	ld hl,kill_drop_dy
	call ADD_HL_A
	ld a,(ix+003h)          ; pixel Y
	sub (hl)                ; lift by table[type]
	ld e,a
	ld d,(ix+005h)          ; pixel X
kill_drop_dy:
...
; ---------------------------------------------------------------------------
flame_init:
	ld (ix+00bh),pose_flame_0       ; +0x0B anim frame = flame sprite 0x85
...
reward_sat_col:                    ; (seg2 0x9C60) SAT colour cycle for actor_reward
	defb 008h,001h,00eh,001h
; actor_floor_test (seg2 0x9C64): carry if map_solid_pair at actor (Y,X).
actor_floor_test:
	ld e,(ix+003h)          ; pixel Y
	ld d,(ix+005h)          ; pixel X
	jp map_solid_pair
...
; spawn_rate_gate (seg2 0x9CCA): Z when this generator should fire.  HL -> the
; CF00+ slot (counter, phase).  DE -> an 8-byte period table.  Shared CF10
; ticks every frame; only every 4th frame decrements the slot.  Reload is
; table[phase&7] minus D012*2 (min 1).  Z = fire this frame.
spawn_rate_gate:
	exx
	ld hl,spawn_slot_tick   ; CF10 shared divider
	dec (hl)
	ld a,(hl)
	exx                     ; restore generator HL / table DE
	and 003h
	ret nz                  ; skip 3 of 4 frames
	dec (hl)                ; per-generator counter
	ret nz                  ; still cooling
	inc l                   ; -> phase/side
	ld a,(hl)
	inc (hl)                ; bump phase
	dec l                   ; back to counter
	and 007h                ; 8-entry table
	call ADD_DE_A           ; DE -> period byte
	ld a,(0d012h)           ; difficulty 0..3
	add a,a                 ; *2
	ld c,a
	ld a,(de)
	sub c                   ; faster as D012 rises
	jr nc,spawn_rate_store
	xor a                   ; floor at 0 before the +1
...
spawn_pick_pos:
	ld a,(simon_y)          ; Simon Y (used by some stage branches)
	ld c,a
...
merman_spawn:
	...
	call ADD_HL_A
	ld d,(hl)
	ld a,(simon_x)          ; skip if Simon X is within 0x18 of spawn X
	sub d
	add a,018h
	cp 030h
	jp nc,spawn_actor
...
spawn_edge_gate:
	ld a,(simon_x)
	cp 0c0h
	jr nc,spawn_edge_right  ; Simon in right third
	cp 040h
	jr c,spawn_edge_left    ; Simon in left third
	ret                     ; middle: always allow
spawn_edge_left:
	ld a,d                  ; spawn X
	cp 040h
	ret nc                  ; spawn not on left: allow
	ld d,0f0h               ; unused (callers ret c first)
spawn_edge_ok:
	ret
spawn_edge_right:
	ld a,d                  ; spawn X
...
spawn_edge_right_ccf:
	ccf                     ; CY if spawn X >= 0xC0
	ret nc                  ; spawn not on right: allow
spawn_edge_right_x:
	ld d,010h               ; unused (callers ret c first)
	ret
...
shot_spawn:
	...
	push ix
	call shot_alloc         ; fill a free D700 slot from CFF*
	pop ix
	ret
...
;  Regen: tools/workbench/msx/regen-bank.sh 3 0xA000
; ===========================================================================
...
aim_scale:
	ld a,(0cff0h)           ; speed
	ld h,a
	call MUL_H_E            ; HL = speed * E
	xor a                   ; A:HL <<= 3  ->  DE = HL / 32
	add hl,hl
	adc a,a
	add hl,hl
	adc a,a
...
; skull_pile_face (seg3 0xA2AF): pose 5 (right) or 4 (left) from Simon X, or
; 7/6 in stage 9 room 4 (the darker pile).
skull_pile_face:
	ld b,005h               ; face right
	ld a,(stage)
	cp 009h
	...
	cp 004h
	jr nz,skull_pile_face_id
	ld b,007h               ; s9r4 variant
skull_pile_face_id:
	ld a,(simon_x)
	cp (ix+005h)
	jr nc,skull_pile_face_store
	dec b                   ; Simon to the left: 4 or 6
...
play_sound_alive:
	ld c,a                  ; keep the sound id
	ld a,(simon_action)
	cp act_dying
	ret z                   ; mute during death anim
	ld a,c
	jp play_sound
...
simon_y_overlap:
	...
	cp 010h
	ret
; actor_flip_xvel (seg3 0xA649): negate X velocity (preserve AF/DE).
actor_flip_xvel:
	push af
	push de
	ld e,(ix+009h)          ; Xvel lo
	ld d,(ix+00ah)          ; Xvel hi
	call neg_de
	call actor_set_xvel
	pop de
	pop af
...
raven_hold:
	call raven_bounds
	ret c
; raven_flap (seg3 0xA79E): pose 0x8A/8B (right) or 0x87/88 (left), toggled
; every 8 ticks of ix+12 from Xvel sign.
raven_flap:
	ld hl,raven_flap_r      ; 0x8A/8B
	bit 7,(ix+00ah)         ; Xvel sign
	jr z,raven_flap_hl      ; rightward
	ld hl,raven_flap_l      ; 0x87/88
raven_flap_hl:
	inc (ix+012h)           ; flap timer
	ld a,(ix+012h)
	and 008h                ; bit 3 -> frame 0/1
	sra a
	sra a
	sra a                   ; A = 0 or 1
	call ADD_HL_A
...
raven_pick_vel:
	ld de,0fe00h            ; left Xvel
	ld hl,00200h            ; right Xvel
	ld a,(simon_x)
	cp (ix+005h)
	jr c,raven_pick_x       ; Simon left: keep DE left
	ex de,hl                ; else right
raven_pick_x:
	ld a,(ix+005h)
	cp 0c0h
	jr nc,raven_pick_right  ; bounce at right edge
	cp 040h
	jr nc,raven_pick_xvel   ; middle: keep toward-Simon
	ld de,00200h            ; bounce at left: force right
	jr raven_pick_xvel
raven_pick_right:
	ld de,0fe00h            ; force left
raven_pick_xvel:
	call actor_set_xvel
	ld hl,raven_coast_dy_up
	ld a,(simon_y)
	cp (ix+003h)
	jr c,raven_pick_yidx    ; Simon above
	ld hl,raven_coast_dy_down
raven_pick_yidx:
	ld a,(ix+018h)
	and 003h                ; 4-entry lut
	add a,a                 ; word index
	call ADD_HL_A
	ld d,(hl)               ; Yvel hi (big-endian)
	inc hl
	ld e,(hl)
	ld a,(ix+003h)
	bit 7,d
	...
	cp 040h
	call c,neg_de           ; too high: invert
	jp actor_set_yvel
raven_pick_y_down:
	cp 0c0h
	call nc,neg_de          ; too low: invert
	jp actor_set_yvel
...
ADD_HL_A_SIGNED:
	bit 7,a
	jp z,ADD_HL_A           ; non-negative: unsigned add
	add a,l
	ld l,a
	ret c                   ; no borrow
	dec h                   ; signed: borrow from H
	ret
; raven_bounds (seg3 0xA834): carry = hit screen edge; force hover (state 2).
; raven_bounds (seg3 0xA834): SCF if the raven hit a screen edge (Y 0x20/0xC0
; or X 0x10/0xE8, using the velocity sign).  Hit: ix+1E=0x18, state=2.
raven_bounds:
	ld a,(ix+003h)          ; pixel Y
	bit 7,(ix+008h)         ; Yvel sign
	jr nz,raven_bounds_up   ; rising -> test top
	cp 0c0h                 ; falling: floor Y
...
raven_bounds_x:
	...
	cp 0e8h                 ; right edge
	jr nc,raven_bounds_hit
	xor a                   ; NC = still inside
	ret
raven_bounds_left:
	cp 010h                 ; left edge
	ret nc                  ; still inside
raven_bounds_hit:
	ld (ix+01eh),018h       ; bounce timer
...
zombie_floor:
	...
	call actor_set_xvel_speedup
	ld (ix+006h),001h
; actor_snap_y8 (seg3 0xA9A9): pixel Y &= 0xF8 (8px nametable row).  Used on
; landing so walkers sit on the tile grid.
actor_snap_y8:
	ld a,(ix+003h)          ; pixel Y
	and 0f8h                ; 8px grid
	ld (ix+003h),a
...
bone_dragon_spit_done:
	...
	ld (ix+001h),001h
	ret
; bone_dragon_sat_nudge (seg3 0xAA6E): 8 SAT bytes at HL, each ±1 toward DE.
; Stride 5 (SAT cell).  Used while the dragon interpolates into place.
bone_dragon_sat_nudge:
	ld b,008h               ; 8 cells
...
enemy_dracula_tick:
	...
	ld de,00000h
	call actor_set_xvel
	ld (ix+00bh),pose_dracula_intro_0
	ld (ix+00ch),01eh
	xor a
	ld (ix+010h),a
	ld (ix+00eh),a
...
dracula_drop:
	...
	ld (ix+003h),a
	ld (ix+006h),000h
	ld (ix+00bh),pose_dracula_stand_l
	ld a,01eh
	ld (ix+00ch),a
...
dracula_fireballs:
	...
	ld (ix+00ch),01eh
	jr dracula_next
; dracula_spawn_fireball (seg3 0xAC90): D700 kind 0x11 from (X, Y-24) with
; Xvel ±0x0280 toward Simon.  HL is Yvel (callers fire 0 / +0x0180 / -0x0180).
dracula_spawn_fireball:
	ld a,(simon_x)
	cp (ix+005h)
	ld de,00280h            ; +X if Simon is to the right
	jr nc,dracula_fireball_vel
	ld de,0fd80h            ; -X
dracula_fireball_vel:
	ld a,(ix+003h)
	sub 018h                ; 24 px above the figure
	ld c,a
	ld b,(ix+005h)          ; X
	ld a,011h               ; kind = Dracula fireball
	jp shot_spawn
dracula_rise:
	dec (ix+00ch)
	ret nz
...
; dracula_sat_color (seg3 0xAD4C): paint 8 SAT colour bytes from
; dracula_sat_cols (0x02 / 0x48 repeating) via dracula_sat_color_from.
dracula_sat_color:
	ld b,008h               ; 8 cells
	ld de,00025h            ; D=col index, E=SAT+0x25 colour field
...
; dracula_sat_hide (seg3 0xAD62): colour 0 on all 8 cells (rise / idle flicker).
dracula_sat_hide:
	ld b,008h               ; 8 SAT cells
	ld e,025h               ; first cell colour offset
...
dracula_sat_write_color:
	...
	ld e,a
	ret
; dracula_save_bg (seg3 0xAD87): HMMM 32x32 from playfield (X-16, Y=0x91)
; page 0 to page 1 (0x80,0x80), covering the last torso slot.
dracula_save_bg:
	ld a,(ix+005h)          ; figure X
	sub 010h                ; dest X = X-16
	ld h,a
	ld l,091h               ; dest Y
	ld de,08080h            ; page-1 stash
	ld bc,02020h            ; 32x32
	ld a,004h               ; HMMM page0->1
	jp vdp_hmmm
...
dracula_blit_torso:
	ld hl,dracula_torso     ; CE0E
	ld a,(hl)
	dec a                   ; 0 = idle (skip); 1..5 -> 0..4
	ret m
	ld (hl),000h            ; consume the slot
	ld de,dracula_torso_src ; (SX,SY) words
	call lookup_word_tbl    ; HL = source
	ex de,hl
	ld a,(dracula_x)        ; CE0F figure X
	sub 010h                ; dest X = X-16
	ld d,a
	ld e,091h               ; dest Y
	ld bc,02020h            ; 32x32
	ld a,h
	cp 080h                 ; slot 5 = saved BG at (0x80,0x80)
...
dracula_torso_hide:
	...
	ld (dracula_torso),a
	ret
; dracula_torso_from_shape (seg3 0xADB5): odd ix+0C -> hide torso (slot 5);
; else map pose 0x5A..0x5E to CE0E blit index.
dracula_torso_from_shape:
	bit 0,(ix+00ch)         ; odd timer: hide cloak
	jr nz,dracula_torso_hide
; dracula_torso_select (seg3 0xADBA): if pose is 0x5A..0x5E, store pose-0x5A
; in CE0E so dracula_blit_torso can LMMM/HMMM that cloak slot.  Else leave it.
dracula_torso_select:
	ld a,(ix+00bh)          ; pose
	sub 05ah                ; 0x5A..0x5E -> 0..4
	cp 005h
	ret nc                  ; not a torso pose
	ld (dracula_torso),a    ; CE0E blit slot
	ret
...
red_skel_wake_go:
	...
	ld (ix+006h),001h
	ret
; red_skel_face (seg3 0xAFA2): pose and Xvel toward Simon (with speedup).
red_skel_face:
	ld a,(simon_x)
	sub (ix+005h)
	jr c,red_skel_face_left
	ld (ix+012h),000h       ; face right
	ld (ix+00bh),pose_red_skel_walk_r0
	ld de,00220h
	jr red_skel_face_xvel
red_skel_face_left:
	ld (ix+012h),001h       ; face left
...
hanging_bat_hang:
	ld (ix+00bh),pose_hanging_bat_hang
	ld (ix+006h),000h
	ld a,(simon_y)
	sub (ix+003h)
	cp 050h
	ret nc
	ld c,pose_hanging_bat_fly_l0
	ld a,(simon_x)
	sub (ix+005h)
	ld de,00180h
...
; actor_add_yvel_a0 (seg3 0xB345): Yvel += 0x00A0 (hunchback air / axe-knight fall).
actor_add_yvel_a0:
	ld de,000a0h
	jp actor_add_yvel
...
blob_edge_right:
	...
	ccf
	ret
; blob_wall_ahead (seg3 0xB5B5): actor_wall_* in the Xvel direction.
blob_wall_ahead:
	ld e,(ix+003h)          ; pixel Y
	ld d,(ix+005h)          ; pixel X
	ld a,d
	or e
	ret z                   ; at origin: skip
	bit 7,(ix+00ah)         ; Xvel sign
...
blob_rise_x:
	...
	ld d,a
	jp map_solid_at
; blob_floor (seg3 0xB5E7): carry if falling onto solid at the feet.
blob_floor:
	xor a
	bit 7,(ix+008h)         ; rising (Yvel negative)?
	ret nz                  ; yes -> not landing
	ld e,(ix+003h)          ; pixel Y
	ld d,(ix+005h)          ; pixel X
	jp map_solid_at
blob_drop:
	...
	ld (ix+001h),001h
	ret
; blob_chase_x (seg3 0xB607): Xvel ±0x01C0 toward Simon.
blob_chase_x:
	ld a,(simon_x)
	cp (ix+005h)
	ld de,001c0h            ; right
...
blob_chase_xvel:
	jp actor_set_xvel
; blob_set_pose (seg3 0xB618): pose from a per-stage packed-nibble row.
; lookup_word_tbl on blob_store_pose+1 overlaps the 16-byte pose map so blob
; stages yield pointers into the packed rows; room/2 picks the byte, odd
; room the low nibble.  Then blob_pose_tbl[nibble], plus 1 if ix+0C bit4.
blob_set_pose:
	ld a,(stage)
	ld de,blob_store_pose+1 ; overlapping word table
	call lookup_word_tbl    ; DE -> packed row
	ld a,(room)
	srl a                   ; room/2
	push af                 ; carry = odd room
	call ADD_DE_A
	pop af
	ld a,(de)               ; packed byte
	jr c,blob_pose_nibble   ; odd room: low nibble
	rra
	rra
	rra
	rra                     ; even: high nibble
blob_pose_nibble:
	and 00fh
	ld hl,blob_pose_tbl     ; nibble -> pose id
	call ADD_HL_A
	ld a,(hl)
	inc (ix+00ch)           ; anim timer
	bit 4,(ix+00ch)
...
orb_sat_on:                     ; apply SAT colour, dec timer
	...
	inc (ix+001h)
	ret
; orb_apply_sat (seg3 0xB7AD): AND C into five SAT colour bytes for this orb
; frame.  Pose 0x8F+ selects the 5-byte row; stage 3 uses orb_sat_s3, else
; orb_sat_rest.  C=0xFF = full colour, C=0 = hide (blink).
orb_apply_sat:
	ld a,(stage)
	ld de,orb_sat_s3        ; giant-bat (stage 3) colours
	sub 003h
	jr z,orb_sat_src
	ld de,orb_sat_rest      ; every other boss
orb_sat_src:                    ; DE = stage-3 or rest colour table
	...
	add a,a
	add a,b                 ; *5
	call ADD_DE_A           ; DE -> 5 colour bytes
	push ix
	pop hl
	ld a,025h               ; SAT colour field
	add a,l
	ld l,a
	ld b,005h               ; 5 cells
orb_sat_cell:                   ; AND C into five SAT colour bytes
	...
	and c                   ; 0 = blink off
	ld (hl),a
	ld a,005h               ; next SAT cell
	add a,l
	ld l,a
	djnz orb_sat_cell
	ld (hl),000h            ; terminator
	ret
; orb_sat_s3 / orb_sat_rest (seg3 0xB801 / 0xB810): 3 frames x 5 SAT colour
;  bytes.  Stage 3 (giant bat) uses orb_sat_s3; every other boss uses orb_sat_rest.
orb_sat_s3:
	defb 00eh,008h,006h,002h,000h
	defb 00eh,008h,006h,004h,002h
	defb 008h,006h,004h,002h,000h
orb_sat_rest:
	defb 00eh,008h,007h,002h,000h
	defb 00eh,008h,007h,005h,002h
	defb 008h,007h,005h,002h,000h
orb_settle:
	call orb_spin
	ld e,(ix+003h)
	ld d,(ix+005h)
...
; mummy_walk_anim (seg3 0xB9D6): 3-frame walk.  ix+12 bit0 picks mummy_walk_l
; (0x33-35) vs mummy_walk_r (0x36-38).  ix+13 is the frame timer (step every 8);
; ix+14 is the pose index 0..2 ping-ponging via ix+15 direction.
mummy_walk_anim:
	ld hl,mummy_walk_r      ; 0x36/37/38
	bit 0,(ix+012h)         ; facing
	jr z,mummy_anim_tick    ; right
	ld hl,mummy_walk_l      ; 0x33/34/35
mummy_anim_tick:
	inc (ix+013h)           ; frame timer
	ld a,(ix+013h)
	cp 008h
	jr nz,mummy_anim_store  ; hold this pose
mummy_anim_step:
	ld (ix+013h),000h       ; reload
	bit 0,(ix+015h)         ; ping-pong dir
	jr z,mummy_anim_back
	inc (ix+014h)           ; next frame
	ld a,(ix+014h)
	cp 003h
mummy_anim_fwd_end:
	jr nz,mummy_anim_store
	ld (ix+014h),001h       ; bounce off 2
	ld (ix+015h),000h       ; go back
	jr mummy_anim_store
mummy_anim_back:
	dec (ix+014h)
	ld a,(ix+014h)
	cp 0ffh
	jr nz,mummy_anim_store
	ld (ix+014h),001h       ; bounce off 0
	ld (ix+015h),001h       ; go fwd
mummy_anim_store:
	ld a,(ix+014h)          ; 0..2
	call ADD_HL_A
	ld a,(hl)
...
igor_land_rng:
	...
	ld (ix+001h),a
	ret
; igor_flash (seg3 0xBBC7): while ix+10 (invis timer) is nonzero, hide physics
; (+06=0) and SCF so the caller skips the rest of the tick.  Else show and NC.
igor_flash:
	ld a,(ix+010h)          ; invis timer
	and a
	jr z,igor_flash_show
	dec (ix+010h)
	ld (ix+006h),000h       ; skip integrate
	scf                     ; still flashing
	ret
igor_flash_show:
	ld (ix+006h),001h       ; visible
	ret
igor_try_throw:
	dec (ix+012h)
	ret nz
	...
	call aim_at_simon
	jp shot_throw
; igor_arm_throw (seg3 0xBBE8): reload ix+12 throw cooldown to 0x30.
igor_arm_throw:
	ld (ix+012h),030h       ; next toss in 48 frames
	ret
; igor_set_pose (seg3 0xBBED): pose C, or C+3 if Xvel is positive (face right).
igor_set_pose:
	bit 7,(ix+00ah)         ; Xvel sign
	ld a,c
	jr nz,igor_pose_store   ; left: keep C
	add a,003h              ; right: C+3
igor_pose_store:
	ld (ix+00bh),a
	ret
...
; grim_sickle_spawn (seg3 0xBC30): next of 8 fixed (Y,X) pairs in grim_sickle_xy,
; cycling CF20.  shot_spawn kind 0x16 (grim) -> D700 type 8, vel 0.
grim_sickle_spawn:
	ld hl,0cf20h            ; sickle index
	ld a,(hl)
	inc (hl)                ; next slot next time
	and 007h                ; 8 positions
	add a,a                 ; word index
...
enemy_medusa_tick:
	...
	ld (ix+007h),010h
	ld (ix+008h),a
	ld (ix+00bh),pose_medusa_0
	ld (ix+006h),001h
	ld (ix+015h),03ch
	ld (ix+014h),008h
	ret
; medusa_bob (seg3 0xBD4D): flicker pose 0x2B/0x2C; Yvel ±0x0020 around Y=0xA0.
medusa_bob:
	dec (ix+014h)           ; bob timer
	ld c,pose_medusa_0
	bit 3,(ix+014h)         ; every 8 frames
	jr z,medusa_bob_pose
	inc c                   ; alternate frame
medusa_bob_pose:
	ld (ix+00bh),c
	ld de,00020h            ; sink
	ld a,(ix+003h)
